...
19/09/2026 — A comprovação automática aceita a grade publicada vinculada a um calendário anterior somente quando uma revisão conjunta aplicada da versão vigente contém a própria turma e explica a agenda atual, com decisão, aplicação e hash coerentes. A âncora da cadeia integra a memória; qualquer alteração não rastreada, aula cancelada no período ou cadeia incompleta volta a exigir confirmação pedagógica. Uma revisão posterior legitimamente aplicada produz nova âncora, sem perpetuar a invalidação da fotografia anterior. A cobertura não cria cobrança e o caminho de confirmação humana permanece disponível.

21/09/2026 — A cadeia pós-replanejamento passa a aceitar a evolução PREVISTO→MINISTRADO no mesmo horário (conclusão pelo diário) sem perder a âncora; qualquer outro status continua divergente. Alterações de quantidade de aulas da modalidade aprovadas e aplicadas depois do replanejamento são encadeadas como elos: cada uma precisa partir exatamente da agenda deixada pelo elo anterior (`agendaAntes`), com decisão, aplicação e hash coerentes, e os encontros adicionados são reconhecidos pelo horário. Os elos integram a âncora (`alteracoesQuantidade`) e o hash de contexto; a data de aplicação do replanejamento apenas ordena os elos e fica fora do hash, preservando memórias já confirmadas. Encontro sem explicação, elo ilegível, rejeitado, sem aplicação ou anterior ao replanejamento volta a exigir confirmação da Gestão. Limites: encontro adicionado por uma alteração e movido por outra posterior não é reconhecido (cai na confirmação humana); transferência no meio do ciclo continua `VINCULO_AUSENTE`; o helper de destinos (`academico/destino-agenda.ts`) ainda não compartilha esta cadeia. Verificação: 34/34 unitários em três arquivos, TypeScript e ESLint sem erros; integração com banco não executada.

21/09/2026 — Destinos acadêmicos (`academico/destino-agenda.ts`) passam a compartilhar a cadeia: quando a agenda futura não coincide diretamente com o replanejamento aplicado, os elos de quantidade aprovados e aplicados são conferidos apenas sobre a agenda futura (`desde`), e a fotografia v3 registra `alteracoesQuantidade` (campo opcional — memórias anteriores continuam válidas). Alteração sem aplicação ou encontro futuro sem explicação mantém a turma indisponível como destino. Verificação: regressão unitária 2.089/2.089 em 397 arquivos, TypeScript sem erros; integração com banco não executada.
