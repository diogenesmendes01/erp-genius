...
## Q92 — Consumo de antecipação por ocorrência cobrável, 18/09/2026

Falta cobrável e cancelamento tardio passam a consumir a reserva antecipada elegível do mesmo encontro e matrícula dentro da própria transação da conferência financeira. O consumo aponta para a ConferenciaOcorrenciaHoras, não possui diário ou presença e conserva o valor proporcional pago na compra original. O preço contratual vigente continua somente informativo na memória; não repricifica saldo, crédito de encerramento ou recebimento já registrado.

O banco exige exatamente uma fonte para o consumo, vincula reserva, compra, encontro, matrícula, minutos e conferente, recusa reserva liberada ou já consumida e exige o consumo atômico quando a conferência cobrável tem reserva. A fonte conferida fica fechada contra versão posterior de OcorrenciaParticular. Realização com diário segue no percurso anterior. Fechamento classifica a destinação como antecipação já conferida, sem nova cobrança; encerramento mostra FALTA_COBRAVEL ou CANCELAMENTO_TARDIO_COBRAVEL, em vez de AULA_REALIZADA.

A tela financeira identifica a reserva/compra consumida e o painel da compra expõe a origem por ocorrência. A integração real usa encontros futuros reservados e espera seu término antes de registrar falta/tardio: cobre os dois desfechos, repetição concorrente, valor histórico, ausência de diário/caixa/cobrança, fechamento, encerramento, XOR, consumo atômico e tentativa SQL de revisar fonte já conferida. Migration 20260918120000_consumo_horas_ocorrencia. Não abrange cancelamento no prazo/escola, que permanece no fluxo de liberação aprovado, nem permuta.

### Consulta integral de pedidos de encerramento — 19/09/2026

A página de movimentações permite consultar pedidos além dos cinquenta mais recentes, com páginas de cinquenta, ordenação estável por criação e identificador e navegação por aluno. A consulta recebe a identidade somente do servidor após a guarda da página e revalida usuário ativo, papel e escopo. Preservar os contratos associados, histórico e ações financeiras existentes. Implementação b82b3658: cinco testes unitários/SSR, duas integrações e TypeScript aprovados no integrador. A fixture histórica não comprova o fluxo de criação/cancelamento dos pedidos; revisão independente acompanhada no quadro único.
