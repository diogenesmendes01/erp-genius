...
M01 — presença histórica em implementação: a migração 203 prepara proposta imutável ligada à linha `HISTORICO_PRESENCA`, origem, mapa M01, matrícula e uma `AulaDiario` já existente. Secretaria, Gestão Pedagógica ou Administração pode preparar; somente Gestão Pedagógica/Administração diferente decide. A aprovação revalida fotografia, contrato, aluno, vigência histórica, pausa e aula; cria apenas o `RegistroAulaAluno` ausente. Registro docente igual recebe vínculo de comprovação; registro divergente é preservado e encaminhado a correção Q23. A resolução posterior é append-only e só reconcilia o mesmo registro após a correção coincidir, sem criar segunda presença. Migração 203 aplicada somente no banco descartável DEV-email; integração dirigida, callbacks e TypeScript passaram com saída 0. Não representa carga real ou homologação externa.
