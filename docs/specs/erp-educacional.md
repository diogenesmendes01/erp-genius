...
O primeiro passo de aplicação é restrito a linhas `CADASTRO` completas, sem pendência ou colisão. Administração ensaia a fotografia e só aplica a mesma versão após confirmação; cada linha mostra a evidência, destino e motivo. O mapa durável usa exclusivamente `origem` e ID de aluno da fonte. E-mail, documento ou nome já existentes sem mapa bloqueiam e exigem conferência, nunca fusão automática. País precisa corresponder de modo inequívoco a cadastro ativo e o fuso precisa ser válido. O cadastro criado preserva documento como texto, desabilita WhatsApp e comunicações, e não cria conta, consentimento, turma, matrícula, alocação, contrato, cobrança, recebimento ou presença. Alteração posterior do destino ou da fotografia fica divergente, sem sobrescrita.
