...
**F07.3/Q37/Q41–Q43 — quantidade de aulas por modalidade (validada localmente; avisos de quantidade pendentes).** A proposta imutável fotografa o conjunto de turmas, sua meta efetiva, agenda antes/depois, conflitos e condições específicas aprovadas da turma. Outra pessoa com Gerência Pedagógica ou Administração aprova e aplica no mesmo callback, depois de conferir novamente a fotografia. Aumento alcança turmas não finalizadas, inclusive iniciadas e rascunhos; redução altera somente não iniciadas. Turmas concluídas, ministradas e encontros passados permanecem preservados. Rascunho ganha nova versão de grade sem publicação automática. Duração, frequência e dias da grade aprovada de cada turma continuam válidos durante a mudança de meta e são apresentados de forma revisável; snapshots legados sem frequência não criam uma exceção artificial. Mudança concorrente da modalidade ou conclusão da turma invalida a proposta, sem decisão, aplicação parcial ou alteração de agenda. As evidências locais são `modalidade-quantidade.int.test.ts` (dez cenários) e `modalidade-quantidade-preview*.test.ts`; a tela de detalhe tem teste SSR. O evento aplicado já contém fotografia de itens para uma futura origem de aviso, mas o guard SQL 182 e qualquer transporte de e-mail/WhatsApp permanecem pendentes de aprovação específica, sem claim de envio ou homologação externa.
