...
P02 — checkpoint local de 18/09/2026: acordo, comprovação pedagógica por período, proposta e decisão independente disponíveis; seleção assistida de matrícula/mensalidades e múltiplos destinos na interface. Valores e propostas financeiros restritos a Financeiro/Administração; Pedagógico confirma serviço sem acesso a esses campos. Decisão ainda NÃO efetiva compensação: aplicação financeira e projeções de saldo seguem pendentes. Evidências e commits no quadro único; P02 permanece em andamento.

P02 — avanço de 18/09/2026: aprovação válida agora aplica atomicamente compensação por destino em registro imutável AplicacaoCompensacaoPermuta e campo separado valorCompensadoPermuta. Reduz saldo sem criar Recebimento ou consumir CréditoMatricula; partial mantém remanescente devido. Migração 223 aplicada somente na integração local. Recebimentos, consulta financeira, uso de crédito e retomada consideram o componente. Restam auditoria dos demais consumidores financeiros (inclusive encerramento/indisponibilidade), concorrência entre acordos e validação independente antes de concluir P02.
