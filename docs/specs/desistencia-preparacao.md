...
Depois de aplicada a primeira memória de um pedido, não pode existir outra aplicação desse mesmo pedido. Informe de pagamento, recebimento destinado, crédito sem destino ou alteração das demais fontes financeiras posterior à aplicação permanece registrado e bloqueia a efetivação pela fotografia divergente. A correção exige uma revisão complementar por delta, ainda não implementada: ela deve referenciar a primeira aplicação, identificar o fato novo, calcular somente a diferença e passar novamente pelas alçadas independentes. Não se deve reaplicar a memória original, apagar o recebimento nem declarar a desistência efetivável enquanto essa pendência existir.

Uma rejeição financeira não apaga a memória preparada. O Financeiro pode reapresentá-la somente como uma nova versão ligada à anterior, com motivo explícito. A reapresentação também pode suceder uma memória aprovada sem aplicação quando a fotografia financeira ou a condição contratual vigente mudou de forma comprovável. A versão anterior permanece preservada com sua decisão; somente a última versão da cadeia pode receber nova decisão ou aplicação. Uma alteração financeira que também invalida o estado do pedido exige nova conferência administrativa Q121: a reapresentação não reutiliza essa aprovação. A cadeia não reabre, não altera nem reaplica uma memória que já produziu a primeira aplicação Q165.
