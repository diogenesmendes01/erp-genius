...
19/09/2026 — A corretiva 269 reconcilia somente o corpo instalado de `preservar_cobranca_cancelada_apos_desistencia` com a versão canônica da migração histórica, sem alterar a migração nem seu checksum. No perfil descartável `dev-email`, o recorte integrado confirma que, após a efetivação, atualização e exclusão de cobrança cancelada continuam recusadas; antes da efetivação, uma cobrança cancelada ainda pode ser atualizada e excluída. O bloqueio posterior preserva o histórico e não transforma o caminho pré-efetivação em erro.
