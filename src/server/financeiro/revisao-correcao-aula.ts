...
export async function consultarRevisoesFinanceirasCorrecaoAula(input: unknown) {
  return executarAcao(async () => {
    ...
    return prisma.$transaction(async tx => {
      await exigirFinanceiroTx(tx, usuario.id);
      const revisoes = await tx.propostaRevisaoFinanceiraCorrecaoAula.findMany({ where: { propostaCorrecaoAula: { encontro: { matriculaId: d.matriculaId } } }, orderBy: [{ propostaCorrecaoAula: { versao: "desc" } }, { versao: "desc" }],
        select: { id: true, versao: true, tipo: true, motivo: true, criadaEm: true, propostaCorrecaoAulaId: true, fotografia: true, preparador: { select: { id: true, nome: true } }, decisao: { select: { id: true, aprovada: true, motivo: true, criadaEm: true, decisor: { select: { id: true, nome: true } } } }, propostaCorrecaoAula: { select: { versao: true, entradaHash: true, encontro: { select: { id: true, inicio: true, fim: true, fusoOrigem: true } } } } } });
      const brutas = await tx.propostaCorrecaoAula.findMany({ where: { encontro: { matriculaId: d.matriculaId }, rejeicao: null, aprovacao: null }, orderBy: { versao: "desc" }, distinct: ["encontroId"], take: 25, select: { id: true, encontroId: true, versao: true, encontro: { select: { id: true, inicio: true, fim: true, fusoOrigem: true } } } });
