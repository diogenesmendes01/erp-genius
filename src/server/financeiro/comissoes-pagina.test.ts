...
const m = vi.hoisted(() => ({ findMany: vi.fn(), count: vi.fn(), sessao: vi.fn() }));
vi.mock("@/lib/prisma", () => ({ prisma: { comissao: { findMany: m.findMany, count: m.count } } }));
vi.mock("@/server/_shared", async (original) => ({ ...(await original<object>()), exigirSessaoComPapel: m.sessao }));

import { COMISSOES_POR_PAGINA, listarComissoesPagina } from "./consultas";
