...
const FotoSchema = z.object({ tipo: z.literal("AULA_NAO_COBRAVEL"), conferencia: z.object({ id: z.string() }), encontro: z.object({ matriculaId: z.string() }), efeito: EfeitoSchema }).passthrough();
 ...
 * a fatura está paga, ou só marca a aula como não cobrável para o próximo fechamento.
...
export async function aplicarRevisaoNaoCobravelTx(tx: Prisma.TransactionClient, input: { aprovacaoId: string; decisaoRevisaoId: string; autorId: string }) {
  ...
  if (e.tipo !== "SEM_ITEM") await tx.$queryRaw`SELECT id FROM "Cobranca" WHERE id=${e.cobrancaId} FOR UPDATE`;
  const aplicacao = await tx.aplicacaoRevisaoFinanceiraCorrecaoAula.create({ data: {
    decisaoRevisaoId: decisao.id, aprovacaoCorrecaoAulaId: input.aprovacaoId, conferenciaId: foto.conferencia.id, matriculaId,
    cobrancaId: e.tipo === "SEM_ITEM" ? null : e.cobrancaId, efeito: e.tipo, valorAula: e.valorAula, moeda: e.moeda,
    versaoCobrancaAntes: e.tipo === "SEM_ITEM" ? null : e.versaoCobranca, valorNegociadoAnterior: e.tipo === "SEM_ITEM" ? null : e.valorNegociadoAnterior,
    ...
    fotografiaHash: decisao.proposta.fotografiaHash,
  } });
  ...
  await registrarEvento(tx, { tipo: "CorrecaoAulaNaoCobravelAplicada", agregadoTipo: "Matricula", agregadoId: matriculaId, autorId: input.autorId,
    payload: { aplicacaoId: aplicacao.id, aprovacaoCorrecaoAulaId: input.aprovacaoId, decisaoRevisaoId: decisao.id, conferenciaId: foto.conferencia.id, efeito: e.tipo,
      valorAula: e.valorAula.toFixed(2), moeda: e.moeda, cobrancaId: e.tipo === "SEM_ITEM" ? null : e.cobrancaId, creditoId } });
  return { id: aplicacao.id, efeito: e.tipo, creditoId };
}
