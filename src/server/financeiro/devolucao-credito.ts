...
export async function decidirDevolucaoCredito(input: z.input<typeof Decidir>) {
  return executarAcao(async () => {
    ...
    return prisma.$transaction(async tx => {
      let p = await tx.propostaDevolucaoCredito.findUnique({ where: { id: d.propostaId }, include: { credito: { select: { matriculaId: true } }, decisao: { include: { reserva: true } } } }); if (!p) throw new ErroRegra("Proposta não encontrada.");
      ...
      p = await tx.propostaDevolucaoCredito.findUniqueOrThrow({ where: { id: d.propostaId }, include: { credito: { select: { matriculaId: true } }, decisao: { include: { reserva: true } } } });
