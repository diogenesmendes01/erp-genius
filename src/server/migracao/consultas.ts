...
export async function consultarLotePreparacaoMigracao(loteId: string) {
  return executarAcao(async () => {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const lote = await tx.lotePreparacaoMigracao.findUnique({ where: { id }, select: { id: true, origem: true, chaveLote: true, estado: true, criadoEm: true, preparadoPor: { select: { nome: true } }, conflitosEntrada: { orderBy: { criadoEm: "asc" }, select: { linhaOrigem: true, codigo: true, entradaHash: true, dadosConflitantes: true, criadoEm: true } }, linhas: { orderBy: { linhaOrigem: "asc" }, select: { id: true, linhaOrigem: true, alunoOrigemId: true, turmaOrigemId: true, matriculaOrigemId: true, financeiroOrigemId: true, dadosOrigem: true, entradaHash: true, estado: true, aplicacoesCadastro: { orderBy: { criadoEm: "asc" }, select: { situacao: true, alunoId: true, detalhe: true, criadoEm: true, executadoPor: { select: { nome: true } } } }, pendencias: { orderBy: [{ campo: "asc" }, { codigo: "asc" }], select: { campo: true, codigo: true, detalhe: true } }, colisoesComoConflitante: { select: { id: true, tipo: true, identificadorOrigem: true, linhaExistente: { select: { linhaOrigem: true } } } }, colisoesComoExistente: { select: { id: true, tipo: true, identificadorOrigem: true, linhaConflitante: { select: { linhaOrigem: true } } } } } } } });
      if (!lote) return null;
