...
export async function consultarPresencasHistoricasMigracao(linhaId:string) {
  return executarAcao(async()=>{const autor=await exigirSessaoComPapel(Papel.SECRETARIA_ACADEMICA,Papel.GERENTE_PEDAGOGICO,Papel.ADMINISTRADOR);return prisma.$transaction(async tx=>{const [u]=await tx.$queryRaw<{ativo:boolean;papeis:Papel[]}[]>(Prisma.sql`SELECT ativo,papeis FROM "Usuario" WHERE id=${autor.id} FOR SHARE`);if(!u?.ativo||!u.papeis.some(p=>p===Papel.SECRETARIA_ACADEMICA||p===Papel.GERENTE_PEDAGOGICO||p===Papel.ADMINISTRADOR))throw new ErroPermissao("Exige contexto acadêmico ativo.");return tx.$queryRaw(Prisma.sql`SELECT p.id,p.status,p.participacao,p.evidencia,p.snapshot,p."criadoEm",p."motivoDecisao",p."preparadorId",p."aulaId",u.nome AS "preparadorNome",d.nome AS "decisorNome",a.id AS "aplicacaoId",a.resultado,a."registroExistenteId",r.id AS "resolucaoId",r.resultado AS "resultadoResolucao",r.evidencia AS "evidenciaResolucao",r."criadoEm" AS "resolvidaEm",ru.nome AS "resolvedorNome",au."ocorridaEm",au.conteudo,t.nome AS turma,al."primeiroNome" || COALESCE(' ' || al.sobrenome,'') AS "alunoNome",m.id AS "matriculaId" FROM "PropostaPresencaHistoricaMigracao" p JOIN "Usuario" u ON u.id=p."preparadorId" JOIN "Matricula" m ON m.id=p."matriculaId" JOIN "Aluno" al ON al.id=m."alunoId" JOIN "AulaDiario" au ON au.id=p."aulaId" LEFT JOIN "Turma" t ON t.id=au."turmaId" LEFT JOIN "Usuario" d ON d.id=p."decisorId" LEFT JOIN "AplicacaoPresencaHistoricaMigracao" a ON a."propostaId"=p.id LEFT JOIN "ResolucaoDivergenciaPresencaHistoricaMigracao" r ON r."aplicacaoId"=a.id LEFT JOIN "Usuario" ru ON ru.id=r."resolvedorId" WHERE p."linhaId"=${linhaId} ORDER BY p.versao DESC`);});});
}

export async function listarAulasElegiveisPresencaHistoricaMigracao(linhaId:string) {
  return executarAcao(async()=>{const autor=await exigirSessaoComPapel(Papel.SECRETARIA_ACADEMICA,Papel.GERENTE_PEDAGOGICO,Papel.ADMINISTRADOR);return prisma.$transaction(async tx=>{const [u]=await tx.$queryRaw<{ativo:boolean;papeis:Papel[]}[]>(Prisma.sql`SELECT ativo,papeis FROM "Usuario" WHERE id=${autor.id} FOR SHARE`);if(!u?.ativo||!u.papeis.some(p=>p===Papel.SECRETARIA_ACADEMICA||p===Papel.GERENTE_PEDAGOGICO||p===Papel.ADMINISTRADOR))throw new ErroPermissao("Exige contexto acadêmico ativo.");return tx.$queryRaw<{id:string;ocorridaEm:Date;conteudo:string;turma:string|null}[]>(Prisma.sql`SELECT DISTINCT a.id,a."ocorridaEm",a.conteudo,t.nome AS turma FROM "LinhaPreparacaoMigracao" l JOIN "LotePreparacaoMigracao" lo ON lo.id=l."loteId" JOIN "MapaOrigemMatriculaMigracao" mm ON mm.origem=lo.origem AND mm."matriculaOrigemId"=l."matriculaOrigemId" JOIN "Matricula" m ON m.id=mm."matriculaId" JOIN "AlocacaoTurma" al ON al."matriculaId"=m.id AND al."alunoId"=m."alunoId" JOIN "AulaDiario" a ON a."turmaId"=al."turmaId" LEFT JOIN "Turma" t ON t.id=a."turmaId" WHERE l.id=${linhaId} AND l."tipoEntrada"='HISTORICO_PRESENCA'::"TipoEntradaPreparacaoMigracao" AND alocacao_cobre_instante(al,a."ocorridaEm" AT TIME ZONE 'UTC') AND situacao_matricula_no_instante(m.id,a."ocorridaEm")='ATIVA' AND a."ocorridaEm" AT TIME ZONE 'UTC'<=clock_timestamp() ORDER BY a."ocorridaEm" DESC`);});});
}
