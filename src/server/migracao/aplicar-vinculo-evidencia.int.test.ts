...
it("rejeita evidências com chave ou texto em branco antes de iniciar a aplicação", async () => {
  const entrada = {
    linhaId: "linha-inexistente", ensaioId: "ensaio-inexistente", entradaHash: "a".repeat(64), contextoHash: "b".repeat(64),
    ...
    evidenciaContrato: { referencia: "contrato legado" }, evidenciaPagamento: { referencia: "pagamento legado" },
    fatos: [{ tipo: "ATIVACAO", data: "2025-01-01", evidencia: { referencia: "fonte" } }],
  };
  for (const alteracao of [
    { evidenciaContrato: { "   ": "documento" } },
    { evidenciaPagamento: { referencia: "  \t " } },
    { fatos: [{ ...entrada.fatos[0], evidencia: { " ": true } }] },
    { fatos: [{ ...entrada.fatos[0], evidencia: { referencia: "\n" } }] },
  ]) expect(await aplicarVinculoMigracao({ ...entrada, ...alteracao })).toMatchObject({ ok: false, erro: expect.stringContaining("evidência não pode estar em branco") });
  expect(await prisma.matricula.count()).toBe(0);
  expect(await prisma.aplicacaoVinculoMigracao.count()).toBe(0);
});
