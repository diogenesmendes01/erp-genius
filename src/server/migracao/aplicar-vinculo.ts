...
export async function aplicarVinculoMigracao(input: unknown) {
 return executarAcao(async () => {
  ...
  return prisma.$transaction(async tx => {
   ...
   const [repetida] = await tx.$queryRaw<{id:string;matriculaId:string;alocacaoId:string;ensaioId:string;snapshot:unknown}[]>(Prisma.sql`SELECT id,"matriculaId","alocacaoId","ensaioId",snapshot FROM "AplicacaoVinculoMigracao" WHERE "linhaId"=${linha.id} AND "entradaHash"=${d.entradaHash} FOR SHARE`);
   if (repetida) {
    if (repetida.ensaioId !== d.ensaioId || JSON.stringify(canonico(repetida.snapshot)) !== JSON.stringify(canonico(snapshot))) throw new ErroRegra("A linha já foi aplicada com condições diferentes. Consulte a aplicação original.");
    return { id:repetida.id, matriculaId:repetida.matriculaId, alocacaoId:repetida.alocacaoId, repetida:true };
   }
