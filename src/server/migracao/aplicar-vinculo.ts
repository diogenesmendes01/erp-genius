...
export async function aplicarVinculoMigracao(input: unknown) {
 return executarAcao(async () => {
  ...
  return prisma.$transaction(async tx => {
   ...
   const alocacaoId=randomUUID(); await tx.alocacaoTurma.create({data:{id:alocacaoId,alunoId:mapaAluno.alunoId,matriculaId,turmaId:turma.turmaId,ativa:status.statusDestino==="ATIVA" || (!fim && status.statusDestino==="PAUSADA"),provenienciaVinculo:"MIGRACAO",inicioVigencia:inicio,fimVigencia:fim}});
