...
export async function decidirImpactosCoberturaAditivo(input: unknown) { return executarAcao(async () => {
 ...
 return prisma.$transaction(async tx => { const db = tx; const ref = await db.conjuntoImpactosCoberturaAditivo.findUniqueOrThrow({ where: { id: d.conjuntoId }, select: { matriculaId: true } }); await bloquearMatriculas(tx,[ref.matriculaId]); await exigirFinanceiro(tx,autor.id,true); const c = await db.conjuntoImpactosCoberturaAditivo.findUniqueOrThrow({ where:{id:d.conjuntoId}, include:{decisao:true,impactos:true} }); if(c.preparadorId===autor.id) throw new ErroRegra("A decisão exige outro Financeiro."); if(c.decisao) { if(c.decisao.decisorId!==autor.id || c.decisao.aprovada!==d.aprovada || c.decisao.chaveIdempotencia!==d.chaveIdempotencia) throw new ErroRegra("O conjunto já recebeu outra decisão."); return {id:c.decisao.id,aprovada:c.decisao.aprovada}; } if(d.aprovada) { await carregarFormalizacao(tx,c.matriculaId,c.propostaAditivoId,(await tx.conferenciaFinalAditivo.findUniqueOrThrow({where:{id:c.conferenciaFinalId}})).conclusaoId,(c.fotografia as any).revisaoHash); const ids=c.impactos.map((i:any)=>i.cobrancaId).sort(); await tx.$queryRaw`SELECT id FROM "Cobranca" WHERE id=ANY(${ids}::text[]) ORDER BY id FOR UPDATE`; const atuais=await tx.cobranca.findMany({where:{id:{in:ids}}}); if(atuais.length!==c.impactos.length || c.impactos.some((i:any)=>hashSubstituicao(foto(atuais.find(x=>x.id===i.cobrancaId)!)) !== (c.fotografia as any).cobrancas.find((x:any)=>x.id===i.cobrancaId).fotografiaHash)) throw new ErroRegra("Mensalidade mudou após o preparo; prepare novo conjunto."); }
