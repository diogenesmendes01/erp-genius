...
const exigeFluxoProprio = new Set(["TAXA_VALOR", "TAXA_VENCIMENTO", "PRIMEIRA_MENSALIDADE_VENCIMENTO", "COBERTURA_INICIO", "COBERTURA_FIM", "ADIANTAMENTO_VALOR", "ADIANTAMENTO_MINUTOS", "ADIANTAMENTO_VENCIMENTO", "MOEDA", "REGIME"]);
...
export function planejarEfeitosAditivo(entrada: readonly AlteracaoEfeitoAditivo[] | unknown, contexto?: { cobrancaEmitida: boolean }) {
  ...
  for (const a of alteracoes) {
    ...
    else if (a.origem === "REGIME") { efeitos.push({ origem: a.origem, dominio: "CONTRATUAL", destino: "condicoes.regime", valor: a.valorEstruturado }); efeitos.push({ origem: a.origem, dominio: "CONDICOES_FUTURAS", destino: "condicoes.regime", valor: a.valorEstruturado }); pendencias.push({ origem: a.origem, mensagem: "Mudança de regime exige fluxo próprio antes da aplicação operacional." }); }
    else if (a.origem === "AGENDA_PARTICULAR") { efeitos.push({ origem: a.origem, dominio: "AGENDA", destino: "agenda.particular.fotografia", valor: a.valorEstruturado }); }
    else if (financeiro.has(a.origem)) { efeitos.push({ origem: a.origem, dominio: "CONDICOES_FUTURAS", destino: `condicoes.${a.origem}`, valor: a.valorEstruturado }); if (exigeFluxoProprio.has(a.origem)) pendencias.push({ origem: a.origem, mensagem: "Esta alteração exige fluxo próprio antes da aplicação operacional." }); if (contexto?.cobrancaEmitida === true) { efeitos.push({ origem: a.origem, dominio: "COBRANCA_EMITIDA", destino: `cobrancas.${a.origem}`, valor: a.valorEstruturado, exigeAcerto: true }); pendencias.push({ origem: a.origem, mensagem: "Selecione e confira as cobranças afetadas antes de qualquer acerto." }); } else if (contexto?.cobrancaEmitida === undefined) pendencias.push({ origem: a.origem, mensagem: "Confira se há cobrança emitida antes de planejar qualquer acerto." }); }
