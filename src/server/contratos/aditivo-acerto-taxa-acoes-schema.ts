...
export const InvalidarAcertoTaxaSchema = z.object({ propostaId: id, motivo: z.string().trim().min(5).max(2000), evidencia: z.record(z.unknown()).refine(v => Object.keys(v).length > 0), chaveIdempotencia: chave }).strict();
