...
export async function decidirAcertoTaxaAditivo(input: unknown) { return executarAcao(async () => {
 const autor = await exigirSessaoComPapel(Papel.FINANCEIRO), d = DecidirAcertoTaxaSchema.parse(input); return prisma.$transaction(async tx => { await financeiro(tx, autor.id, true); await tx.$queryRaw`SELECT id FROM "PropostaAcertoTaxaAditivo" WHERE id=${d.propostaId} FOR UPDATE`; let p = await tx.propostaAcertoTaxaAditivo.findUniqueOrThrow({ where: { id: d.propostaId }, include: { decisao: true } }); await tx.$queryRaw`SELECT id FROM "Matricula" WHERE id=${p.matriculaId} FOR UPDATE`; p = await tx.propostaAcertoTaxaAditivo.findUniqueOrThrow({ where: { id: d.propostaId }, include: { decisao: true } });
