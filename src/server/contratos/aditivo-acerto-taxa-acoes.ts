...
function foto(estado: { revisaoHash: string }, v: { id: string; condicoesHash: string }, c: { id: string; versao: number; moeda: string; valorNegociado: Prisma.Decimal; valorRecebido: Prisma.Decimal | null; valorLiquidadoCredito: Prisma.Decimal; vencimento: Date }, calc: ReturnType<typeof calcularCreditoAcertoTaxa>) {
 return { revisaoHash: estado.revisaoHash, condicoesHash: v.condicoesHash, versaoCondicoesId: v.id, cobranca: { id: c.id, versao: c.versao, moeda: c.moeda, valorNegociado: c.valorNegociado.toFixed(2), valorRecebido: c.valorRecebido?.toFixed(2) ?? null, valorLiquidadoCredito: c.valorLiquidadoCredito.toFixed(2), vencimento: c.vencimento.toISOString() }, calculo: { creditoAnterior: calc.creditoAnterior.toFixed(2), creditoNovo: calc.creditoNovo.toFixed(2) } };
}
async function revalidar(tx: Prisma.TransactionClient, p: { matriculaId: string; propostaAditivoId: string; conferenciaFinalId: string; versaoCondicoesId: string; cobrancaId: string; fotografiaHash: string; valorNovo: Prisma.Decimal; creditoAnterior: Prisma.Decimal; creditoNovo: Prisma.Decimal }) {
 ...
 const calc = calcularCreditoAcertoTaxa({ valorRecebido: c.valorRecebido, valorLiquidadoCredito: c.valorLiquidadoCredito, valorNovo: p.valorNovo, creditosTaxaJaOriginados: anteriores._sum.valor ?? 0 });
 if (calc.pendencia || !calc.creditoAnterior.eq(p.creditoAnterior) || !calc.creditoNovo.eq(p.creditoNovo) || hashSubstituicao(foto(estado, v, c, calc)) !== p.fotografiaHash) throw new ErroRegra("A fotografia do acerto ficou obsoleta; prepare novamente.");
 return c;
}
...
export async function proporAcertoTaxaAditivo(input: unknown) { return executarAcao(async () => {
 ...
 return prisma.$transaction(async tx => { await tx.$queryRaw`SELECT id FROM "Matricula" WHERE id=${d.matriculaId} FOR UPDATE`; await financeiro(tx, autor.id);
  ...
  const c = await tx.cobranca.findFirst({ where: { id: d.cobrancaId, matriculaId: d.matriculaId, tipo: "MATRICULA" } }); if (!v || !c) throw new ErroRegra("Selecione uma cobrança de taxa real da matrícula."); await tx.$queryRaw`SELECT id FROM "Cobranca" WHERE id=${c.id} FOR UPDATE`;
  ...
  if (calc.pendencia) throw new ErroRegra(`${calc.pendencia.tratamento} Valor a conciliar: ${calc.pendencia.valor.toFixed(2)}.`); const fotografia = foto(estado, v, c, calc), fotografiaHash = hashSubstituicao(fotografia);
  const repetida = await tx.propostaAcertoTaxaAditivo.findUnique({ where: { preparadorId_chaveIdempotencia: { preparadorId: autor.id, chaveIdempotencia: d.chaveIdempotencia } } }); if (repetida) { if (repetida.fotografiaHash !== fotografiaHash) throw new ErroRegra("A chave idempotente já foi usada com outra proposta."); return { id: repetida.id, status: repetida.status }; }
...
export async function aplicarAcertoTaxaAditivo(input: unknown) { return executarAcao(async () => {
 const autor = await exigirSessaoComPapel(Papel.FINANCEIRO), d = AplicarAcertoTaxaSchema.parse(input); return prisma.$transaction(async tx => { await financeiro(tx, autor.id, true); const p = await tx.propostaAcertoTaxaAditivo.findUniqueOrThrow({ where: { id: d.propostaId }, include: { decisao: true, aplicacao: true } }); await tx.$queryRaw`SELECT id FROM "Matricula" WHERE id=${p.matriculaId} FOR UPDATE`; await tx.$queryRaw`SELECT id FROM "PropostaAcertoTaxaAditivo" WHERE id=${p.id} FOR UPDATE`;
