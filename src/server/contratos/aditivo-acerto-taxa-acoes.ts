...
import { calcularCreditoAcertoTaxa, saldoLiquidoAcertoTaxa } from "./aditivo-acerto-taxa-calculo";
...
async function revalidar(tx: Prisma.TransactionClient, p: { matriculaId: string; propostaAditivoId: string; conferenciaFinalId: string; versaoCondicoesId: string; cobrancaId: string; fotografiaHash: string; valorNovo: Prisma.Decimal; creditoAnterior: Prisma.Decimal; creditoNovo: Prisma.Decimal }) {
 ...
 if (!calc.creditoAnterior.eq(p.creditoAnterior) || !calc.creditoNovo.eq(p.creditoNovo) || hashSubstituicao(foto(estado, v, c, calc, comissoes)) !== p.fotografiaHash) throw new ErroRegra("A fotografia do acerto ficou obsoleta; prepare novamente.");
 return c;
}
...
export async function proporAcertoTaxaAditivo(input: unknown) { return executarAcao(async () => {
 ...
 return prisma.$transaction(async tx => { await financeiro(tx, autor.id); await bloquearMatriculas(tx, [d.matriculaId]);
  const anterior = await tx.propostaAcertoTaxaAditivo.findUnique({ where: { preparadorId_chaveIdempotencia: { preparadorId: autor.id, chaveIdempotencia: d.chaveIdempotencia } }, include: { conferenciaFinal: { select: { conclusaoId: true } } } });
  if (anterior) {
    const fotoAnterior = anterior.fotografia as { revisaoHash?: string };
    if (anterior.matriculaId !== d.matriculaId || anterior.propostaAditivoId !== d.propostaAditivoId || anterior.conferenciaFinal.conclusaoId !== d.conclusaoId || anterior.cobrancaId !== d.cobrancaId || fotoAnterior.revisaoHash !== d.revisaoHash || anterior.motivo !== d.motivo || JSON.stringify(anterior.evidencia) !== JSON.stringify(d.evidencia)) throw new ErroRegra("A chave idempotente já foi usada com outra proposta.");
  ...
  const comissoes = await tx.comissao.findMany({ where: { matriculaId: d.matriculaId }, orderBy: { id: "asc" } }), fotografia = foto(estado, v, c, calc, comissoes), fotografiaHash = hashSubstituicao(fotografia);
