import { revisarConferenciaRegraHistorica, proporConferenciaRegraHistorica, decidirConferenciaRegraHistorica } from "./conferencia-regra-historica";
...
it("cobre somente os trechos M01 ativos e dentro da vigência, sem ativadaEm comercial", async () => {
  ...
  const preparado = await prepararLoteMigracao({ origem: "PLANILHA", chaveLote: "m01-lancamento-historico", linhas: [{
    ...
    turma: { id: "turma-1", codigo: "M01-AV" }, matricula: { id: "matricula-1", produtoOrigem: "produto-1", situacao: "ATIVA", inicio: "2025-01-01", moeda: "CRC", pais: "CR" }, alocacao: { inicio: "2025-01-01", fim: "2025-02-01" }, consentimentoOrigem: "fonte",
  }] });
  ...
  const aplicada = await aplicarVinculoMigracao({ linhaId: linha.id, ensaioId: ensaio.dado.id, entradaHash: linha.entradaHash, contextoHash, fusoReferencia: "America/Costa_Rica", semanticaFim: "LIMITE_EXCLUSIVO", inicioAlocacao: "2025-01-01", fimAlocacao: "2025-02-01", diaVencimento: 10, mesesPlano: 9, evidenciaContrato: { arquivo: "contrato" }, evidenciaPagamento: { arquivo: "pagamento" }, fatos: [{ tipo: "ATIVACAO", data: "2025-01-01", evidencia: { etapa: "ativacao" } }, { tipo: "PAUSA", data: "2025-01-10", evidencia: { etapa: "pausa" } }, { tipo: "ATIVACAO", data: "2025-01-20", evidencia: { etapa: "retomada" } }] });
  ...
  expect(await prisma.matricula.findUniqueOrThrow({ where: { id: aplicada.dado.matriculaId }, select: { ativadaEm: true } })).toEqual({ ativadaEm: null });
  const cobre = async (inicio: string, fim: string) => prisma.$queryRaw<{ cobre: boolean }[]>`
    SELECT situacao_autorizacao_segunda_chamada_cobre_intervalo(
      ${aplicada.dado!.matriculaId}, ${alocacaoHistoricaId}, ${"I1"},
      ${instanteUtcSql(new Date(inicio))}, ${instanteUtcSql(new Date(fim))}
    ) AS cobre
  `;
  expect(await cobre("2024-12-31T12:00:00.000Z", "2024-12-31T13:00:00.000Z")).toEqual([{ cobre: false }]);
  expect(await cobre("2025-01-05T12:00:00.000Z", "2025-01-09T12:00:00.000Z")).toEqual([{ cobre: true }]);
  expect(await cobre("2025-01-10T12:00:00.000Z", "2025-01-19T12:00:00.000Z")).toEqual([{ cobre: false }]);
  await prisma.autorizacaoEspecialSegundaChamada.create({ data: {
    matriculaId: aplicada.dado.matriculaId, alocacaoId: alocacaoHistoricaId, regraId: regra.id, codigoAvaliacao: "I1", autorizadorId: aprovador.id,
    motivo: "Autorização histórica conferida para o trecho pausado", prazoAte: new Date("2025-01-19T12:00:00.000Z"),
    chaveIdempotencia: "m01-autorizacao-historica", entradaHash: "c".repeat(64), criadaEm: new Date("2025-01-11T12:00:00.000Z"),
  } });
  expect(await cobre("2025-01-10T12:00:00.000Z", "2025-01-19T12:00:00.000Z")).toEqual([{ cobre: false }]);
  expect(await cobre("2025-01-11T12:00:00.000Z", "2025-01-19T12:00:00.000Z")).toEqual([{ cobre: true }]);
  expect(await cobre("2025-01-20T12:00:00.000Z", "2025-02-01T06:00:00.000Z")).toEqual([{ cobre: true }]);
  expect(await cobre("2025-02-01T06:00:00.000Z", "2025-02-01T07:00:00.000Z")).toEqual([{ cobre: false }]);
  ...
  await expect(prisma.$transaction(tx => salvarLancamentoTx(tx, professor.id, { alocacaoId: alocacaoHistoricaId, codigoAvaliacao: "I1", realizadaEm: "2025-01-22T12:00:00.000Z", notas: [{ habilidade, nota: "8", comentarioAluno: "Registro histórico conferido" }], submetida: true, versaoEsperada: 0, chaveIdempotencia: "m01-lancamento-historico" }))).resolves.toMatchObject({ versao: 1 });
});
