...
it("frequência usa vigência migrada, pausa e encerramento operacional sem duplicar a trajetória", async () => {
  ...
  if (!aplicadaA.ok || !aplicadaA.dado || !aplicadaB.ok || !aplicadaB.dado) throw new Error("Aplicações ausentes.");
  const professor = await criarUsuario([Papel.PROFESSOR], "Professor frequência");
  const criarAula = async (turmaId: string, inicio: string) => {
    const data = new Date(inicio);
    const encontro = await prisma.encontroAgenda.create({ data: { turmaId, professorId: professor.id, preparadorId: professor.id, inicio: data, fim: new Date(data.getTime() + 3600000), fusoOrigem: "UTC", status: "PREVISTO", motivo: "Aula histórica", chaveIdempotencia: `freq-${turmaId}-${inicio}`, entradaHash: "fixture", diario: { create: { turmaId, professorId: professor.id, ocorridaEm: data, conteudo: "Aula", registros: { create: { alunoId: aluno.id, matriculaId: aplicadaA.dado!.matriculaId, nomeAluno: "Ana Lima", presente: true, participacao: "PRESENTE" } } } } } });
    await prisma.encontroAgenda.update({ where: { id: encontro.id }, data: { status: "MINISTRADO" } });
    return encontro;
  };
  const jan = await criarAula(turmaA.id, "2025-01-15T12:00:00Z");
  const pausa = await criarAula(turmaA.id, "2025-02-15T12:00:00Z");
  const bValida = await criarAula(turmaB.id, "2025-04-15T12:00:00Z");
  await prisma.alocacaoTurma.update({ where: { id: aplicadaB.dado.alocacaoId }, data: { ativa: false, encerradaEm: new Date("2025-04-20T00:00:00Z") } });
  const aposEncerramento = await criarAula(turmaB.id, "2025-04-25T12:00:00Z");
  const nivelFrequencia = await prisma.$transaction(tx => carregarFrequenciaNivelTx(tx, { matriculaId: aplicadaA.dado!.matriculaId, nivelId: nivel.id, minimoPercentual: "75", agora: new Date("2026-01-01T00:00:00Z") }));
  expect(nivelFrequencia).toMatchObject({ base: 2, presencas: 2, escopo: "MATRICULA_NIVEL" });
  expect(nivelFrequencia.memoria.map(aula => aula.aulaId)).toEqual([jan.id, bValida.id]);
  expect(nivelFrequencia.memoria.map(aula => aula.aulaId)).not.toContain(pausa.id);
  expect(nivelFrequencia.memoria.map(aula => aula.aulaId)).not.toContain(aposEncerramento.id);
  const alocacaoA = await prisma.alocacaoTurma.findUniqueOrThrow({ where: { id: aplicadaA.dado.alocacaoId } });
  const frequenciaVinculo = await prisma.$transaction(tx => carregarFrequenciaVinculoTx(tx, { ...alocacaoA, matriculaId: aplicadaA.dado!.matriculaId }, nivel.id, "75", new Date("2026-01-01T00:00:00Z")));
  expect(frequenciaVinculo).toMatchObject({ base: 1, presencas: 1, alocacaoId: alocacaoA.id });
});
