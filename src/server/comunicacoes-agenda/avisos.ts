...
export async function criarAvisosAlteracaoAgendaTx(tx: Prisma.TransactionClient, entrada: { eventoId: string; matriculaId: string; encontrosIds: string[] }) {
  ...
  const matricula = await tx.matricula.findUnique({ where: { id: entrada.matriculaId }, select: { id: true, alunoId: true, aluno: { select: { email: true, telefoneE164: true, whatsapp: true, aceitaComunicacoes: true } } } });
  ...
  const pertenceAMatricula = (encontro: typeof encontros[number]) => encontro.matriculaId === matricula.id || (substituicao && !!encontro.turmaId && alocacoes.some((alocacao) =>
  ...
  if (matricula.aluno.whatsapp && matricula.aluno.telefoneE164) canais.push({ canal: "WHATSAPP", contato: matricula.aluno.telefoneE164 });
  const avisos = [];
  for (const { canal, contato } of canais) {
    const existente = await tx.avisoAlteracaoAgenda.findFirst({ where: { eventoId: entrada.eventoId, matriculaId: matricula.id, canal } });
    if (existente) {
...
export async function despacharAvisoAlteracaoAgendaInterna(id: string, entregar: EntregarAvisoAgenda) {
  const preparado = await prisma.$transaction(async tx => {
    ...
    const fonteValida = (fonteRemarcacao || fonteSubstituicao) && aviso.matricula?.status === "ATIVA" && aviso.itens.length > 0 && aviso.itens.every(pertenceAMatricula);
...
export async function processarAvisosAlteracaoAgenda(entregar: EntregarAvisoAgenda) {
  ...
  if (!process.env.RESEND_API_KEY?.trim() || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(process.env.EMAIL_INSTITUCIONAL_REMETENTE?.trim() ?? "")) return 0;
...
export async function entregarAvisoAlteracaoAgenda(entrada: { canal: Canal; destinatario: string; avisoId: string; encontrosIds: string[] }) {
  ...
  if (!(fonteRemarcacao || fonteSubstituicao) || !aviso.itens.every(itemValido)) return { situacao: "RECUSADO" as const };
