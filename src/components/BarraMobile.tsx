...
// Alvos de toque ≥ 40px (p-2 em ícone de 24px).
export function BarraMobile({ papeis, nome, naoLidasInbox = 0 }: { papeis: string[]; nome: string; naoLidasInbox?: number }) {
  ...
  const botao = "rounded-md p-2 text-gray-500 hover:bg-gray-100 hover:text-gray-800";

  return (
    <>
      <header className="sticky top-0 z-40 flex items-center gap-1 border-b border-gray-200 bg-surface px-2 py-1.5 md:hidden">
        <button type="button" className={botao + " relative"} aria-label="Abrir menu" aria-expanded={aberta} onClick={() => setAberta(true)}>
