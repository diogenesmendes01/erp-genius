...
export function AplicarCadastroMigracao({ loteId }: { loteId: string }) {
  ...
  async function executar(modo: "ENSAIO" | "APLICAR") {
    ...
    try {
      ...
      router.refresh();
    } catch { setResultado(MSG_RESULTADO_INCERTO_SEM_CHAVE); } finally { setOcupado(false); }
  }
  return <div className="rounded border border-amber-200 bg-amber-50 p-3 text-sm"><strong>Aplicação limitada a cadastros de alunos</strong><p className="mt-1">Ensaio e aplicação só usam linhas CADASTRO completas, sem pendências ou colisões. Turmas, matrículas, financeiro, consentimentos, presença e contas continuam sem aplicação.</p><div className="mt-2 flex gap-2"><button type="button" className={botaoClasses({ variante: "secundario", tamanho: "md" })} disabled={ocupado} onClick={() => executar("ENSAIO")}>{ocupado ? "Processando…" : "Ensaiar cadastros"}</button><button type="button" className={botaoClasses({ tamanho: "md" })} disabled={ocupado || !confirmacaoHash} onClick={() => executar("APLICAR")}>{ocupado ? "Processando…" : "Aplicar cadastros confirmados"}</button></div>{!confirmacaoHash && <p className="mt-2 text-xs">A aplicação só libera depois de um ensaio desta fotografia.</p>}<MensagemStatus texto={resultado} className="mt-2" />{linhas.length > 0 && <ul className="mt-2 space-y-1 text-xs">{linhas.map((linha) => <li key={linha.linhaOrigem}><strong>{linha.linhaOrigem}</strong>: {linha.situacao ?? "sem evidência"}{linha.alunoId ? ` · destino ${linha.alunoId}` : ""}{linha.detalhe ? ` · ${linha.detalhe}` : ""}</li>)}</ul>}</div>;
}
