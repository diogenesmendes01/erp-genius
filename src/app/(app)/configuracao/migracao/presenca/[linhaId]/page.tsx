...
export default async function PresencaHistoricaPage({params}:{params:Promise<{linhaId:string}>}) {
  ...
  if(!aulas.ok)return <section className="max-w-4xl space-y-4"><Link className="text-brand-700 underline" href="/configuracao/migracao">Voltar aos lotes</Link><h2 className="text-lg font-medium">Presença histórica da migração</h2><p role="alert" className="rounded border border-red-200 bg-red-50 p-3">Não foi possível consultar as aulas elegíveis: {aulas.erro}.</p></section>;
  if(!propostas.ok)return <section className="max-w-4xl space-y-4"><Link className="text-brand-700 underline" href="/configuracao/migracao">Voltar aos lotes</Link><h2 className="text-lg font-medium">Presença histórica da migração</h2><p role="alert" className="rounded border border-red-200 bg-red-50 p-3">Não foi possível consultar as propostas: {propostas.erro}.</p></section>;
