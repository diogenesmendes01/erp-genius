...
export default async function ModelosPage({ searchParams }: { searchParams: Promise<{ pagina?: string }> }) {
  ...
  return <section className="space-y-4">
    ...
    {dados.familias.length > 0 && <Link className="underline" href="/configuracao/contratos/novo">Preparar novo modelo</Link>}
    {dados.familias.length === 0 ? <EstadoVazio bloco acao={<Link className={botaoClasses({ variante: "secundario" })} href="/configuracao/contratos/novo">Preparar novo modelo</Link>}>Nenhum modelo cadastrado.</EstadoVazio> : <ul className="space-y-2">{dados.familias.map((f) => <li key={f.codigo} className="rounded border p-3">
