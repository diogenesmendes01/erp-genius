  ...
  return <section className="space-y-3 rounded border p-4">
    ...
    {!documentoId && !fontesOriginaisEnviados.length ? <p className="text-sm">Confirme o contrato ou aguarde o envio externo confirmado de um original para preparar estas condições.</p> : !versoes.some((v) => v.status === "PENDENTE") && <details>
      ...
      <form key={`${documentoId ?? "original"}-${versoes.length}`} className="mt-3 grid gap-3 md:grid-cols-2" onSubmit={(e) => {
        ...
        <label className="grid gap-1 text-sm">Condições dos descontos<CampoTexto name="condicoes" required className={campo} /></label>
        ...
        {tipoMulta === "SEM_PREVISAO" && <label className="grid gap-1 text-sm">Fundamento da ausência de multa<CampoTexto name="semMulta" required className={campo} /></label>}
        {["VALOR_FIXO", "PERCENTUAL"].includes(tipoMulta) && <>
          ...
          <label className="grid gap-1 text-sm">Condições de aplicação<CampoTexto name="aplicacao" required className={campo} /></label>
          ...
          {tipoMulta === "PERCENTUAL" && <label className="grid gap-1 text-sm">Base de cálculo prevista no contrato<CampoTexto name="base" required className={campo} /></label>}
        </>}
