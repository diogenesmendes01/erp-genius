...
export function CondicoesEncerramento({ matriculaId, codigo, documentoId, autorId, administrador, versoes }: {
  matriculaId: string; codigo: string; documentoId: string | null; autorId: string; administrador: boolean; versoes: Versao[];
}) {
  ...
  return <section className="space-y-3 rounded border p-4">
    ...
    <p className="text-sm">Transcreva as condições do contrato confirmado. A aprovação confere estas regras; o acerto financeiro e o encerramento exigem seus próprios procedimentos.</p>
    ...
    {versoes.map((v) => {
      ...
      return <article key={v.id} className="space-y-2 rounded bg-gray-50 p-3 text-sm">
        ...
        {v.status === "PENDENTE" && (administrador && v.preparadorId !== autorId ? <form className="flex flex-wrap items-end gap-2" onSubmit={(e) => {
          ...
          <label className="grid gap-1">Decisão<select name="decisao" required defaultValue="" className={campo}><option value="">Selecione</option><option value="aprovar" disabled={!r}>Aprovar regras conferidas</option><option value="rejeitar">Rejeitar proposta</option></select></label>
    ...
    {!documentoId ? <p className="text-sm">Confirme o contrato antes de preparar estas condições.</p> : !versoes.some((v) => v.status === "PENDENTE") && <details>
      <summary className="cursor-pointer">Preparar nova versão das condições</summary>
      <form key={`${documentoId}-${versoes.length}`} className="mt-3 grid gap-3 md:grid-cols-2" onSubmit={(e) => {
        ...
        const regras = RegrasEncerramentoSchema.safeParse({ diaEncerramento: s("dia"), metodoDesconto: s("desconto"), condicoesDescontos: s("condicoes"), multa });
        if (!regras.success) { setErro("Complete as regras do contrato. Valores devem usar ponto decimal e até duas casas."); return; }
        void executar(() => prepararCondicoesEncerramento({ matriculaId, documentoId, regras: regras.data, motivo: s("motivo") }));
      }}>
