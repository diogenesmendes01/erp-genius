...
export function SecretariaPainel({ secretaria, matriculas }: { secretaria: boolean; matriculas: Matricula[] }) {
  ...
  return <div className="space-y-4">
    ...
    {matriculas.map((m) => <article key={m.id} className="space-y-3 rounded-lg border p-4">
      ...
      {secretaria && !m.assumida && !["ENCERRADA", "CANCELADA"].includes(m.status) && <button disabled={ocupado} onClick={() => executar(() => assumirMatricula(m.id))} className="rounded bg-brand-600 px-3 py-2 text-sm text-white disabled:opacity-50">Assumir matrícula</button>}
      {secretaria && m.assumida && ["RASCUNHO", "AGUARDANDO"].includes(m.status) && <div className="space-y-1"><button disabled={ocupado} onClick={() => executar(() => concluirMatricula(m.id))} className="rounded bg-brand-600 px-3 py-2 text-sm text-white disabled:opacity-50">Concluir matrícula</button><p className="text-xs text-gray-500">Requer aceite contratual, disponibilidade e pagamentos exigidos nas condições da matrícula. A entrada pode exigir primeira mensalidade ou adiantamento das particulares.</p></div>}
