...
export default async function RegularizacoesGravacaoPage({ searchParams }: { searchParams: Promise<{ cursor?: string; busca?: string }> }) {
  ...
  const fontes = [
    ...dados.publicacoes.map((p) => ({ id: `PUBLICACAO_AULA:${p.id}`, rotulo: `Aula ${p.encontro.turma?.codigo ?? p.encontro.turma?.nome ?? p.encontroId} — ${formatarInstanteExibicao(p.encontro.inicio, preferencia.ok ? preferencia.dado?.fusoExibicao : null, p.encontro.fusoOrigem).texto}`, versao: p.fontesRevisao[0]?.versao ?? null })),
  ...
  const propostas = dados.propostas.map((p) => ({ ...p, destino: p.publicacaoAula ? `Aula ${p.publicacaoAula.encontro.turma?.codigo ?? p.publicacaoAula.encontro.turma?.nome ?? p.publicacaoAulaId} — ${formatarInstanteExibicao(p.publicacaoAula.encontro.inicio, preferencia.ok ? preferencia.dado?.fusoExibicao : null, p.publicacaoAula.encontro.fusoOrigem).texto}` : `Reposição ${p.materialReposicao?.reposicao.matricula.codigo ?? p.materialReposicaoId} — ${[p.materialReposicao?.reposicao.matricula.aluno.primeiroNome, p.materialReposicao?.reposicao.matricula.aluno.sobrenome].filter(Boolean).join(" ")}` }));
