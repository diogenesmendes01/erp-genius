...
export function SolicitarReposicao({ origem }: { origem: OrigemReposicao }) {
  ...
  return <form className="space-y-3 rounded border p-4" onChange={() => { chave.current = null; setErro(""); }} onSubmit={e => {
    ...
    <label className="block">Evidência do pedido<CampoTexto name="evidencia" required minLength={5} maxLength={4000} className="block w-full rounded border p-2" /></label>
    ...
    <button className={botaoClasses({ variante: "secundario", tamanho: "lg" })} disabled={ocupado}>{ocupado ? "Registrando…" : "Registrar pedido"}</button>
...
function AgendarParticular({ reposicaoId, opcoes, excecoes, preferenciaFusoExibicao }: { reposicaoId: string; opcoes: NonNullable<ReposicaoEquipe["agendaInicial"]>; excecoes: NonNullable<ReposicaoEquipe["excecoesAgenda"]>; preferenciaFusoExibicao: string | null }) {
  const [ocupado, iniciar] = useTransition(); const [erro, setErro] = useState(""); const [sucesso, setSucesso] = useState(""); const [previa, setPrevia] = useState<PreviaAgenda | null>(null); const [horarioConferido, setHorarioConferido] = useState<HorarioExcecaoProposto | null>(null); const periodo = useInicioDoPeriodo();
  ...
  return <section className="space-y-3 border-t pt-3" aria-label="Ciclo da agenda particular">
    ...
    {!agenda.cancelamentoId && !agenda.remarcacaoId && <div className="grid gap-3 md:grid-cols-2">
      <form className="space-y-2 rounded border p-3" onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); executar(() => proporCancelamentoReposicaoIndividual({ agendaId: agenda.agendaId, motivo: String(f.get("motivo") ?? ""), evidencia: String(f.get("evidencia") ?? ""), chaveIdempotencia: crypto.randomUUID() })); }}>
        <p className="font-medium">Propor cancelamento</p><label className="block">Motivo<CampoTexto name="motivo" required minLength={5} className="block w-full rounded border p-2" /></label><label className="block">Evidência<CampoTexto name="evidencia" required minLength={5} className="block w-full rounded border p-2" /></label><button disabled={ocupado} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Propor cancelamento</button>
      </form>
      <form className="space-y-2 rounded border p-3" onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); executar(() => proporRemarcacaoReposicaoIndividual({ agendaId: agenda.agendaId, professorId: String(f.get("professorId") ?? ""), inicioLocal: String(f.get("inicioLocal") ?? ""), fimLocal: String(f.get("fimLocal") ?? ""), fuso: String(f.get("fuso") ?? ""), motivo: String(f.get("motivo") ?? ""), evidencia: String(f.get("evidencia") ?? ""), chaveIdempotencia: crypto.randomUUID() })); }}>
        <p className="font-medium">Propor remarcação</p><label className="block">ID do professor<input name="professorId" required defaultValue={agenda.professorId ?? ""} className="block w-full rounded border p-2" /></label><label className="block">Início local<input name="inicioLocal" type="datetime-local" required {...periodo.propsInicio} className="block w-full rounded border p-2" /></label><label className="block">Fim local<input name="fimLocal" type="datetime-local" required min={periodo.min} className="block w-full rounded border p-2" /></label><input name="fuso" type="hidden" value={agenda.fusoOrigem} readOnly /><label className="block">Motivo<CampoTexto name="motivo" required minLength={5} className="block w-full rounded border p-2" /></label><label className="block">Evidência<CampoTexto name="evidencia" required minLength={5} className="block w-full rounded border p-2" /></label><button disabled={ocupado} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Propor remarcação</button>
      </form>
    </div>}
...
function DecidirReposicao({ reposicaoId }: { reposicaoId: string }) {
  ...
  return <form className="space-y-3 border-t pt-3" onSubmit={e => {
    ...
    <label className="block">Justificativa<CampoTexto name="motivo" required minLength={5} maxLength={4000} className="block w-full rounded border p-2" /></label>
