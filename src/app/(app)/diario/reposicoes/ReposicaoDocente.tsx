...
const data = (valor: string, fuso: string) => formatarInstanteExibicao(valor, fuso, "UTC").texto;

export function ReposicaoDocente({ reposicaoId, versaoAnterior, modalidade, origem, entrega, encontros, fusoExibicao }: {
  reposicaoId: string; versaoAnterior: number; modalidade: "PARTICULAR" | "GRAVACAO"; origem: OrigemReposicao; entrega: EntregaGravadaPendente | null; encontros?: { id: string; inicio: string; fim: string; fuso: string; status: string; participacao: "PRESENTE" | "FALTA" | "IMPEDIDO_POR_RESTRICAO" | null }[]; fusoExibicao: string;
}) {
  ...
  if (modalidade === "PARTICULAR") {
    ...
    } catch { setErro(MSG_RESULTADO_INCERTO_SEM_CHAVE); } }); }}><fieldset disabled={ocupado} className="space-y-3"><legend className="text-xl font-medium">Realizar reposição particular</legend><p>Origem: {descreverParticipacaoOrigem(origem.participacao)}, de {data(origem.inicio, fusoExibicao)} a {data(origem.fim, fusoExibicao)} (exibido em {fusoExibicao}; origem {origem.fuso}).</p>{avisoOrigemCorrigida}<p>Horário da reposição: {data(encontro.inicio, fusoExibicao)} a {data(encontro.fim, fusoExibicao)} (origem {encontro.fuso}).</p>
      {encontro.status === "PREVISTO" ? <><label className="block">Participação<select name="participacao" defaultValue="" required className="block rounded border p-2"><option value="" disabled>Selecione</option><option value="PRESENTE">Presente</option><option value="FALTA">Falta</option></select></label><label className="block">Conteúdo realizado<CampoTexto name="conteudo" required minLength={5} className="block w-full rounded border p-2" /></label><button className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Registrar diário da particular</button></> : encontro.participacao === "PRESENTE" ? <><p role="status">Diário com presença registrado. Confirme para regularizar somente a ausência de origem.</p><label className="block">Evidência da conclusão<CampoTexto name="evidencia" required minLength={5} className="block w-full rounded border p-2" /></label><button className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Confirmar reposição</button></> : <p role="status">Diário registra {encontro.participacao === "FALTA" ? "falta" : "impedimento"}; a origem permanece sem regularização.</p>}</fieldset>{erro && <p role="alert">{erro}</p>}<MensagemStatus texto={sucesso} /></form>;
  }
  ...
  return <><form className="space-y-3 rounded border p-4" onSubmit={e => {
    ...
    <p>Origem: {descreverParticipacaoOrigem(origem.participacao)}, de {data(origem.inicio, fusoExibicao)} a {data(origem.fim, fusoExibicao)} (exibido em {fusoExibicao}; origem {origem.fuso}). A validação regulariza somente esta aula e preserva o registro original.</p>
    {avisoOrigemCorrigida}
    <details><summary>Entrega do aluno em {data(entrega.entregueEm, fusoExibicao)} (exibido em {fusoExibicao}; origem {origem.fuso})</summary><p className="mt-2 whitespace-pre-wrap">Resumo: {entrega.resumo}</p><p className="mt-2 whitespace-pre-wrap">Atividade: {entrega.atividade}</p><p className="mt-2 whitespace-pre-wrap">Evidência: {entrega.evidencia}</p></details>
    ...
    <label className="block">Evidência da validação<CampoTexto name="evidencia" required minLength={5} maxLength={4000} className="block w-full rounded border p-2" /></label>
    <button name="operacao" value="VALIDAR" className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>{ocupado ? "Validando…" : "Confirmar reposição"}</button>
    <div className="border-t pt-3"><label className="block">Comentário para solicitar correção<CampoTexto name="comentarioCorrecao" minLength={5} maxLength={4000} className="block w-full rounded border p-2" /></label><button name="operacao" value="CORRIGIR" formNoValidate className={`${botaoClasses({ variante: "secundario", tamanho: "lg" })} mt-2`}>Pedir correção ao aluno</button></div>
