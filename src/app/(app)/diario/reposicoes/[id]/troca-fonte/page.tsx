...
export default async function TrocaFonteReposicaoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const dados = await consultarTrocaFonteReposicaoGravacao({ reposicaoId: id });
  return <section className="mx-auto max-w-3xl space-y-5 p-6">
    <Link href="/diario/reposicoes" className="text-sm text-brand-700 underline">Voltar às reposições</Link>
