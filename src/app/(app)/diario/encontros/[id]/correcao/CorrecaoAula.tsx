...
function ElegibilidadeSimulada({ elegibilidade }: { elegibilidade: Elegibilidade }) {
  return <section className="mt-3 rounded border border-gray-200 p-3 text-sm">
    ...
    {elegibilidade.pendencias.length > 0 && <ul className="mt-2 list-disc space-y-1 pl-5 text-amber-700">{elegibilidade.pendencias.map((pendencia) => <li key={pendencia}>{rotulosPendencia[pendencia] ?? "Há uma pendência acadêmica a conferir"}.</li>)}</ul>}
...
function Impactos({ dados }: { dados: DadosImpactos }) {
  ...
  return <section className="space-y-4 rounded border border-amber-200 bg-amber-50 p-4" aria-live="polite">
    ...
    <section className="space-y-2">
      ...
      {dados.comparacao.conteudo.alterado && <div className="rounded border bg-surface p-3 text-sm"><p className="font-medium">Conteúdo ministrado</p><p className="mt-1 whitespace-pre-wrap text-gray-700">Antes: {dados.comparacao.conteudo.antes}</p><p className="mt-1 whitespace-pre-wrap text-gray-700">Proposto: {dados.comparacao.conteudo.depois}</p></div>}
      {dados.comparacao.registros.filter((registro) => registro.participacaoAlterada || registro.observacaoAlterada).map((registro) => <article key={registro.registroId} className="rounded border bg-surface p-3 text-sm">
...
export function CorrecaoAula({ encontroId, dados: dadosIniciais, podeConferirImpactos, fusoExibicao }: { encontroId: string; dados: DadosRevisao; podeConferirImpactos: boolean; fusoExibicao: string }) {
  ...
  const aprovarProposta = (item: PropostaHistorica) => iniciar(async () => {
    ...
    if (impactos.financeiro.exigeConferenciaFinanceira && !impactos.financeiro.revisaoFinanceira?.decisaoAprovadaId) { setErro("Há dependências financeiras que exigem revisão aprovada antes da publicação."); return; }
    ...
    try {
      const resultado = await aprovarCorrecaoAula({ propostaId: item.id, propostaHash: item.propostaHash, impactosHash: impactos.impactosHash, motivo, confirmarPreservacaoReposicoes: avaliacaoReposicoes.todasAutorizadas, revisaoFinanceiraDecisaoId: impactos.financeiro.revisaoFinanceira?.decisaoAprovadaId });
  ...
  return <div className="space-y-5">
    <section className="space-y-3 rounded border bg-[var(--surface)] p-4">
      <div className="flex flex-wrap items-center justify-between gap-3"><h2 className="text-lg font-medium">{dados.podePropor ? "Fonte conferida" : "Fonte registrada"}</h2><button type="button" onClick={recarregar} disabled={ocupado} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Recarregar fonte</button></div>
      ...
      <p className="whitespace-pre-wrap rounded border bg-surface p-3 text-sm">{dados.snapshot.conteudo}</p>
    ...
    {dados.podePropor ? <form className="space-y-4 rounded border bg-[var(--surface)] p-4" onSubmit={(evento) => { evento.preventDefault(); enviarProposta(); }}>
      ...
      <fieldset disabled={ocupado} className="space-y-4">
        <label className="block">Conteúdo ministrado<CampoTexto value={conteudo} onChange={(evento) => { setConteudo(evento.target.value); idempotencia.current = null; setProposta(null); setImpactos(null); }} required minLength={1} maxLength={20000} className="mt-1 block w-full rounded border p-2" /></label>
        ...
        <label className="block">Evidência da conferência<CampoTexto value={evidencia} onChange={(evento) => { setEvidencia(evento.target.value); idempotencia.current = null; setProposta(null); setImpactos(null); }} required minLength={5} maxLength={3000} className="mt-1 block w-full rounded border p-2" /></label>
        ...
        <button type="submit" disabled={ocupado || !houveAlteracao || !!proposta} className={botaoClasses({ tamanho: "lg" })}>{ocupado ? "Registrando…" : "Registrar proposta de correção"}</button>
      </fieldset>
    ...
    {proposta && <section className="space-y-3 rounded border border-blue-200 bg-blue-50 p-4"><h2 className="text-lg font-medium">Proposta registrada</h2><p>Versão {proposta.versao}. A correção ainda não foi publicada e o diário original permanece vigente.</p>{podeConferirImpactos && <button type="button" disabled={ocupado} onClick={() => conferirImpactos(proposta.id)} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>{ocupado ? "Conferindo…" : "Conferir impactos da proposta"}</button>}{!podeConferirImpactos && <p className="text-sm text-gray-700">A conferência de impactos é feita pela gestão pedagógica.</p>}</section>}
    {propostas.length > 0 && <section className="space-y-3 rounded border bg-[var(--surface)] p-4"><header><h2 className="text-lg font-medium">Histórico de propostas</h2><p className="text-sm text-gray-700">As propostas preservam a fonte registrada. Uma proposta no histórico não altera o diário vigente.</p></header>{propostas.map((item) => <article key={item.id} className="space-y-2 rounded border bg-surface p-3"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-medium">Proposta versão {item.versao}</h3><time className="text-sm text-gray-600" dateTime={item.criadaEm}>{formatarInstanteExibicao(item.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time></div><p className="text-sm">Preparada por {item.autor.nome}.</p><p className="whitespace-pre-wrap text-sm"><span className="font-medium">Motivo: </span>{item.motivo}</p><p className="whitespace-pre-wrap text-sm"><span className="font-medium">Evidência: </span>{item.evidencia}</p><ResumoPropostaHistorica proposta={item} />
      {item.aprovacao ? <section className="rounded border border-green-200 bg-green-50 p-3 text-sm"><h4 className="font-medium">Correção publicada</h4><p>Decisão de {item.aprovacao.decisor.nome} em <time dateTime={item.aprovacao.criadaEm}>{formatarInstanteExibicao(item.aprovacao.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time>.</p><p className="mt-1 whitespace-pre-wrap">{item.aprovacao.motivo}</p><p className="mt-1 text-gray-700">A decisão publicada permanece no histórico.</p></section> : item.rejeicao ? <section className="rounded border border-red-200 bg-red-50 p-3 text-sm"><h4 className="font-medium">Proposta rejeitada</h4><p>Decisão de {item.rejeicao.decisor.nome} em <time dateTime={item.rejeicao.criadaEm}>{formatarInstanteExibicao(item.rejeicao.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time>.</p><p className="mt-1 whitespace-pre-wrap">{item.rejeicao.motivo}</p><p className="mt-1 text-gray-700">A rejeição não publica nem altera o diário.</p></section> : <>
        {podeConferirImpactos && <button type="button" disabled={ocupado} onClick={() => { conferirImpactos(item.id); }} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>{ocupado ? "Conferindo…" : "Conferir impactos desta proposta"}</button>}
        {podeConferirImpactos && item.versao === versaoAtualDaConsulta && item.podeRejeitar && impactos?.propostaId === item.id && <section className="space-y-2 rounded border border-amber-200 bg-amber-50 p-3 text-sm"><h4 className="font-medium">Decisão de publicação</h4>{(() => {
          const avaliacaoReposicoes = avaliarReposicoes(impactos);
          return impactos.financeiro.exigeConferenciaFinanceira && !impactos.financeiro.revisaoFinanceira?.decisaoAprovadaId ? <p className="text-amber-700">Há dependências financeiras que exigem revisão aprovada antes da publicação. Os valores não são exibidos nesta tela.</p> : avaliacaoReposicoes.afetadas.length > 0 && !avaliacaoReposicoes.todasAutorizadas ? <p className="text-amber-700">Há reposições relacionadas que precisam de conferência antes da publicação.</p> : propostaParaAprovar !== item.id ? <><p className="text-gray-700">{avaliacaoReposicoes.todasAutorizadas ? avaliacaoReposicoes.temContinuaveis ? `${avaliacaoReposicoes.temConcluidasPreservaveis ? "As reposições concluídas podem ser preservadas sem duplicar a frequência. " : ""}O atendimento autorizado das reposições ainda sem conclusão continua, com agenda, prazo e benefício preservados. Registre a justificativa e confirme essa preservação para publicar a correção.` : "As reposições concluídas podem ser preservadas sem duplicar a frequência. Registre a justificativa e confirme essa preservação para publicar a correção." : "A conferência atual não encontrou dependências que bloqueiem esta decisão. Registre uma justificativa para publicar a correção."}</p><button type="button" disabled={ocupado} onClick={() => { setPropostaParaAprovar(item.id); setMotivoAprovacao(""); setConfirmouPreservacaoReposicoes(false); setErro(""); }} className={botaoClasses({ tamanho: "lg" })}>Publicar correção</button></> : <form className="space-y-2" onSubmit={(evento) => { evento.preventDefault(); aprovarProposta(item); }}><p className="text-gray-700">A publicação aplica a proposta conferida ao diário e registra esta decisão no histórico.</p><label className="block">Justificativa da publicação<CampoTexto value={motivoAprovacao} onChange={(evento) => setMotivoAprovacao(evento.target.value)} required minLength={5} maxLength={3000} disabled={ocupado} className="mt-1 block w-full rounded border p-2" /></label>{avaliacaoReposicoes.todasAutorizadas && <label className="flex items-start gap-2 text-gray-700"><input type="checkbox" checked={confirmouPreservacaoReposicoes} onChange={(evento) => setConfirmouPreservacaoReposicoes(evento.target.checked)} disabled={ocupado} /><span>{textoPreservacaoReposicoes(avaliacaoReposicoes)}</span></label>}<div className="flex gap-2"><button type="submit" disabled={ocupado || (avaliacaoReposicoes.todasAutorizadas && !confirmouPreservacaoReposicoes)} className={botaoClasses({ tamanho: "lg" })}>{ocupado ? "Publicando…" : "Confirmar publicação"}</button><button type="button" disabled={ocupado} onClick={() => { setPropostaParaAprovar(null); setMotivoAprovacao(""); setConfirmouPreservacaoReposicoes(false); }} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Cancelar</button></div></form>;
        })()}</section>}
        {podeConferirImpactos && item.versao === versaoAtualDaConsulta && item.podeRejeitar && propostaParaRejeitar !== item.id && <button type="button" disabled={ocupado} onClick={() => { setPropostaParaRejeitar(item.id); setMotivoRejeicao(""); setPropostaParaAprovar(null); setMotivoAprovacao(""); setErro(""); }} className={botaoClasses({ variante: "perigo", tamanho: "lg" })}>Rejeitar proposta</button>}
        {podeConferirImpactos && item.versao === versaoAtualDaConsulta && item.podeRejeitar && propostaParaRejeitar === item.id && <form className="space-y-2 rounded border border-red-200 bg-red-50 p-3" onSubmit={(evento) => { evento.preventDefault(); rejeitarProposta(item); }}><h4 className="font-medium">Registrar rejeição</h4><p className="text-sm text-gray-700">A rejeição encerra esta proposta sem publicar nenhuma alteração no diário.</p><label className="block text-sm">Motivo da rejeição<CampoTexto value={motivoRejeicao} onChange={(evento) => setMotivoRejeicao(evento.target.value)} required minLength={5} maxLength={3000} disabled={ocupado} className="mt-1 block w-full rounded border p-2" /></label><div className="flex gap-2"><button type="submit" disabled={ocupado} className={botaoClasses({ variante: "perigo", tamanho: "lg" })}>{ocupado ? "Registrando…" : "Confirmar rejeição"}</button><button type="button" disabled={ocupado} onClick={() => { setPropostaParaRejeitar(null); setMotivoRejeicao(""); }} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Cancelar</button></div></form>}
        {!podeConferirImpactos && <p className="text-sm text-gray-600">A gestão pedagógica pode conferir e decidir esta proposta.</p>}
      </>}
    </article>)}{proximaVersao !== null && <button type="button" disabled={ocupado} onClick={carregarPropostasAnteriores} className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>{ocupado ? "Carregando…" : "Carregar propostas anteriores"}</button>}</section>}
