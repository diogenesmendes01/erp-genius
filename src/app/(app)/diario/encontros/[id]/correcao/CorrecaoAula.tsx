...
export function CorrecaoAula({ encontroId, dados: dadosIniciais, podeConferirImpactos, fusoExibicao }: { encontroId: string; dados: DadosRevisao; podeConferirImpactos: boolean; fusoExibicao: string }) {
  ...
  return <div className="space-y-5">
    ...
    {propostas.length > 0 && <section className="space-y-3 rounded border bg-[var(--surface)] p-4"><header><h2 className="text-lg font-medium">Histórico de propostas</h2><p className="text-sm text-gray-700">As propostas preservam a fonte registrada. Uma proposta no histórico não altera o diário vigente.</p></header>{propostas.map((item) => <article key={item.id} className="space-y-2 rounded border bg-surface p-3"><div className="flex flex-wrap items-baseline justify-between gap-2"><h3 className="font-medium">Proposta versão {item.versao}</h3><time className="text-sm text-gray-600" dateTime={item.criadaEm}>{formatarInstanteExibicao(item.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time></div><p className="text-sm">Preparada por {item.autor.nome}.</p><p className="whitespace-pre-wrap text-sm"><span className="font-medium">Motivo: </span>{item.motivo}</p><p className="whitespace-pre-wrap text-sm"><span className="font-medium">Evidência: </span>{item.evidencia}</p><ResumoPropostaHistorica proposta={item} />
      {item.aprovacao ? <section className="rounded border border-green-200 bg-green-50 p-3 text-sm"><h4 className="font-medium">Correção publicada</h4><p>Decisão de {item.aprovacao.decisor.nome} em <time dateTime={item.aprovacao.criadaEm}>{formatarInstanteExibicao(item.aprovacao.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time>.</p><p className="mt-1 whitespace-pre-wrap">{item.aprovacao.motivo}</p><p className="mt-1 text-gray-700">A decisão publicada permanece no histórico.</p></section> : item.rejeicao ? <section className="rounded border border-rose-200 bg-rose-50 p-3 text-sm"><h4 className="font-medium">Proposta rejeitada</h4><p>Decisão de {item.rejeicao.decisor.nome} em <time dateTime={item.rejeicao.criadaEm}>{formatarInstanteExibicao(item.rejeicao.criadaEm, fusoExibicao, "UTC").texto} {fusoExibicao}</time>.</p><p className="mt-1 whitespace-pre-wrap">{item.rejeicao.motivo}</p><p className="mt-1 text-gray-700">A rejeição não publica nem altera o diário.</p></section> : <>
