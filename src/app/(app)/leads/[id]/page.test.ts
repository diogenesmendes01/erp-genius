...
const m = vi.hoisted(() => ({ sessao: vi.fn(), lead: vi.fn(), professores: vi.fn(), preferencia: vi.fn(), configComercial: vi.fn(), sugestoesIA: vi.fn() }));
vi.mock("@/server/_shared", () => ({ exigirSessaoPagina: m.sessao, numeroOuNull: (v: unknown) => v }));
vi.mock("@/server/comercial/consultas", () => ({ obterLead: m.lead, carregarConfigComercial: m.configComercial }));
vi.mock("@/server/ia/consultas", () => ({ sugestoesPendentesDoLead: m.sugestoesIA }));
