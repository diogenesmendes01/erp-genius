...
export function AgendaParticularFormulario({ leadId, ofertaId, versaoOferta, fixa, onChange }: { leadId: string; ofertaId: string; versaoOferta: number; fixa: boolean; onChange: (agenda: AgendaConferida | null) => void }) {
  ...
  return <fieldset disabled={ocupado} className="space-y-3 rounded border p-3"><legend>Agenda particular {fixa ? "com grade fixa" : "flexível"}</legend>
    ...
    <button type="button" onClick={() => buscar(1)} className="rounded border p-2">Buscar professores</button>
    ...
    <button type="button" disabled={horarios.length >= 1000} onClick={() => { setHorarios([...horarios, { data: "", horario: "", minutos: "" }]); limpar(); }} className="rounded border p-2">Adicionar encontro</button>
    <button type="button" onClick={conferir} className="rounded border p-2">Conferir disponibilidade</button>
