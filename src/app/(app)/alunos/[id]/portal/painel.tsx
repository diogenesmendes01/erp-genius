...
type Troca = { id: string; novoEmail: string; situacao: string; preparadorId: string; criadaEm: string; decisao: { aprovada: boolean; motivo: string } | null };

export function PainelAcessoPortalAluno({ alunoId, emailCadastro, conta, trocas, podeDecidir }: {
  alunoId: string; emailCadastro: string | null; conta: { emailVerificado: string | null; temSenha: boolean; ativa: boolean } | null; trocas: Troca[]; podeDecidir: boolean;
}) {
  ...
  return <div className="space-y-5"><section className="rounded border bg-white p-4"><h2 className="font-medium">Situação</h2><dl className="mt-3 grid gap-2 text-sm"><div><dt className="text-gray-500">E-mail cadastrado</dt><dd>{emailCadastro ?? "Ausente"}</dd></div><div><dt className="text-gray-500">Identidade de portal</dt><dd>{conta ? conta.ativa ? conta.temSenha ? "Ativa" : "Aguardando definição de senha" : "Bloqueada" : "Ainda não preparada"}</dd></div><div><dt className="text-gray-500">E-mail verificado do portal</dt><dd>{conta?.emailVerificado ?? "Ainda não confirmado pelo convite"}</dd></div></dl></section>
    <section className="rounded border bg-white p-4"><h2 className="font-medium">Convite individual</h2><p className="mt-1 text-sm text-gray-600">Não há senha nem link para copiar nesta tela.</p><button onClick={convite} className="mt-3 rounded bg-brand-600 px-3 py-2 text-sm text-white">Preparar convite</button></section>
    <section className="rounded border bg-white p-4"><h2 className="font-medium">Troca assistida de e-mail</h2><form onSubmit={troca} className="mt-3 grid gap-3"><label className="text-sm">Novo e-mail<input required name="novoEmail" type="email" className="mt-1 w-full rounded border px-3 py-2" /></label><label className="text-sm">Motivo<textarea required minLength={5} name="motivo" className="mt-1 w-full rounded border px-3 py-2" /></label><label className="text-sm">Evidência de identidade e vínculo<textarea required minLength={5} name="evidencia" className="mt-1 w-full rounded border px-3 py-2" /></label><button className="justify-self-start rounded border px-3 py-2 text-sm">Preparar validação</button></form></section>
    <section className="rounded border bg-white p-4"><h2 className="font-medium">Solicitações de troca</h2>{!trocas.length ? <p className="mt-2 text-sm text-gray-600">Nenhuma solicitação.</p> : <div className="mt-3 space-y-3">{trocas.map((troca) => <article key={troca.id} className="rounded border p-3 text-sm"><p>{troca.novoEmail} · {troca.situacao}</p><p className="mt-1 text-gray-600">Preparada em {new Date(troca.criadaEm).toLocaleString("pt-BR")}</p>{troca.decisao && <p className="mt-1">Decisão: {troca.decisao.aprovada ? "aprovada" : "rejeitada"}.</p>}{podeDecidir && troca.situacao === "PENDENTE_DECISAO" && <form onSubmit={(evento) => { evento.preventDefault(); void decidir(troca.id, true, String(new FormData(evento.currentTarget).get("motivo") ?? "")); }} className="mt-3 flex flex-wrap gap-2"><input required name="motivo" placeholder="Motivo da decisão" className="min-w-48 flex-1 rounded border px-2 py-1" /><button className="rounded bg-brand-600 px-2 py-1 text-white">Aprovar e aplicar</button><button type="button" onClick={(evento) => { const formulario = evento.currentTarget.form; if (formulario) void decidir(troca.id, false, String(new FormData(formulario).get("motivo") ?? "")); }} className="rounded border px-2 py-1">Rejeitar</button></form>}</article>)}</div>}</section>
    {erro && <p role="alert" className="text-sm text-red-700">{erro}</p>}{aviso && <p className="text-sm text-green-700">{aviso}</p>}
  </div>;
}
