...
export function OutrasCobrancasResumo({ snapshot }: { snapshot: unknown }) {
  ...
  return <>{r.data.contratos.map((c) => <section key={c.calculo.matriculaId} className="space-y-2 rounded border p-3">
    ...
    {!c.outrasCobrancasConferidas ? <p>Esta versão não contém conferência das demais cobranças.</p> : <>
      ...
      {c.outrasCobrancasConferidas.parcelas.map((p) => <details key={p.cobrancaId}><summary>Cobrança {p.cobrancaId} · devido proposto {p.valorDevidoProposto} {c.calculo.moeda}</summary>
        <dl className="grid gap-x-4 gap-y-1 sm:grid-cols-2">
          <dt>Valor negociado antes do acerto</dt><dd>{p.valorOriginal} {c.calculo.moeda}</dd>
          <dt>Recebimentos registrados</dt><dd>{p.valorRecebido} {c.calculo.moeda}</dd>
          <dt>Crédito já utilizado nesta cobrança</dt><dd>{p.valorLiquidadoCredito ?? "0.00"} {c.calculo.moeda}</dd>
          <dt>Valor devido proposto</dt><dd>{p.valorDevidoProposto} {c.calculo.moeda}</dd>
          <dt>Saldo a pagar após o acerto</dt><dd>{p.saldoDevido} {c.calculo.moeda}</dd>
          <dt>Crédito apurado pelo acerto</dt><dd>{p.creditoApurado} {c.calculo.moeda}</dd>
        </dl>
        ...
        {p.origemFaturamentoHoras && <details className="mt-2"><summary>Origem das particulares por hora · {p.origemFaturamentoHoras.itens.length} encontro(s)</summary>
          ...
          <ul>{p.origemFaturamentoHoras.itens.map(item => <li key={item.id}>Conferência {item.conferenciaId} · valor faturado preservado {item.valor} {c.calculo.moeda}</li>)}</ul>
        </details>}
        {!!p.recebimentos?.length && <details className="mt-2"><summary>Recebimentos preservados · {p.recebimentos.length}</summary>
          <ul>{p.recebimentos.map(recebimento => <li key={recebimento.id}>Recebimento {recebimento.id} · {recebimento.valor} {recebimento.moeda}</li>)}</ul>
        </details>}
        {!!p.utilizacoesCredito?.length && <details className="mt-2"><summary>Utilizações anteriores de crédito · {p.utilizacoesCredito.length}</summary>
          <ul>{p.utilizacoesCredito.map(uso => <li key={uso.propostaId}>Crédito {uso.creditoId} · {uso.valor} {c.calculo.moeda} · aprovação {uso.decisaoId}</li>)}</ul>
        </details>}
        <p>Motivo: {p.motivo}. Evidência: {p.evidenciaContratual}.</p>
      </details>)}
      {c.outrasCobrancasConferidas.consolidado && <p>Subtotal das demais cobranças: saldo devido {c.outrasCobrancasConferidas.consolidado.saldoDevidoSemCompensarCreditos}; crédito separado {c.outrasCobrancasConferidas.consolidado.creditoApurado} {c.calculo.moeda}.</p>}
    </>}
