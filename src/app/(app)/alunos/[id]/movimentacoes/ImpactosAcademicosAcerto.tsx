...
export function ImpactosAcademicosAcerto({ snapshot, preferenciaFusoExibicao = null }: { snapshot: unknown; preferenciaFusoExibicao?: string | null }) {
  ...
  return <div className="space-y-2"><p>Fuso do acerto: {r.data.fusoInstitucional ?? "ausente nesta versão; prepare nova conferência"}.</p>{r.data.contratos.map((c, i) => {
    ...
    return <details key={a.matriculaId}><summary>Vínculos e agenda da matrícula · {a.vinculos.filter(v => v.ativa).length} vínculo(s) ativo(s)</summary>
      ...
      <ul>{a.encontrosParticulares.map(e => { const x = formatarInstanteExibicao(e.inicio, preferenciaFusoExibicao, e.fusoOrigem); return <li key={e.id}>{x.texto} ({x.fuso}) · {e.status}</li>; })}</ul>
    </details>;
  })}</div>;
}
