  ...
  return <section className="space-y-5">
    ...
    <section className="space-y-3" aria-label="Pedidos de encerramento">
      ...
      {pedidos.map((p) => <article key={p.id} className="space-y-1 rounded border p-3 text-sm">
        ...
        {["ABERTA", "EM_ACERTO", "CONCLUIDA"].includes(p.status) && usuario.papeis.some((papel) => papel === "FINANCEIRO" || papel === "ADMINISTRADOR") && <AcertoEncerramento alunoId={id} solicitacaoId={p.id} matriculas={p.itens.map((i) => i.matricula)} />}
      </article>)}
    </section>
