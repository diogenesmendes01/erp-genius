...
export function LancamentosEncerramento({ snapshot }: { snapshot: unknown }) {
  ...
  return <div>{r.data.contratos.map((c, i) => c.lancamentos?.plano && <details key={i}><summary>Lançamentos previstos do acerto · {c.lancamentos.plano.moeda}</summary>
    <table><thead><tr><th>Cobrança</th><th>Valor após acerto</th><th>Saldo devido</th><th>Crédito apurado</th></tr></thead><tbody>{c.lancamentos.plano.ajustes.map(a => <tr key={a.cobrancaId}><td>{a.cobrancaId}</td><td>{formatarMoeda(a.valorDevido, c.lancamentos!.plano!.moeda)}</td><td>{formatarMoeda(a.saldo, c.lancamentos!.plano!.moeda)}</td><td>{formatarMoeda(a.credito, c.lancamentos!.plano!.moeda)}</td></tr>)}</tbody></table>
    <p>Multa prevista: {c.lancamentos.plano.multa ? formatarMoeda(c.lancamentos.plano.multa.valorProposto, c.lancamentos.plano.moeda) : "conferir"} · Vencimento: {formatarDataCivil(c.lancamentos.plano.multa?.vencimento, "sem cobrança programada")}.</p><p>Créditos previstos por origem:</p><ul>{c.lancamentos.plano.creditos.map(cr => <li key={`${cr.origemTipo}:${cr.origemId}`}>{cr.origemTipo === "COBRANCA" ? "Cobrança" : "Compra de horas"} {cr.origemId}: {formatarMoeda(cr.valor, c.lancamentos!.plano!.moeda)}</li>)}</ul>
