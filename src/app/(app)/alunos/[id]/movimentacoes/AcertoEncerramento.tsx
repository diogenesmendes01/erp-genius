...
function Resumo({ snapshot }: { snapshot: unknown }) {
  ...
  return <div className="space-y-3">{resultado.data.contratos.map(({ calculo: c, propostaExcecaoMulta: excecao, origem, compensacaoFinanceira, horasAntecipadas, consolidacao }) => <div key={c.matriculaId} className="space-y-2 overflow-x-auto rounded bg-gray-50 p-3">
    ...
    <p>Serviço: {c.totalServico} · Multa: {c.multa.valor} · Saldo devido: {c.saldoDevidoSemCompensarCreditos} · Crédito apurado: {c.creditoApuradoSemUtilizacao}</p>
    ...
    {consolidacao && <div className="rounded border p-2"><h4>Prévia consolidada para aprovação</h4>{consolidacao.pendencias.map(p => <p key={p} className="text-amber-800">{p}</p>)}{consolidacao.totais && <p>Saldo devido: {consolidacao.totais.saldoDevidoSemCompensarCreditos} · Crédito apurado separado: {consolidacao.totais.creditoApuradoSemUtilizacao} {c.moeda}. Multa proposta incluída no saldo: {consolidacao.totais.multaProposta}. Nenhuma utilização ou devolução executada.</p>}</div>}{horasAntecipadas && <div><h4>Horas antecipadas</h4>{horasAntecipadas.pendencias.map(p => <p key={p} className="text-amber-800">{p}</p>)}{horasAntecipadas.calculo && <p>Crédito apurado pelas horas restantes: {horasAntecipadas.calculo.creditoApurado} {c.moeda}. Créditos já emitidos foram descontados. Este valor aguarda consolidação e aprovação do acerto.</p>}</div>}{compensacaoFinanceira && <div><h4>Apuração adicional de compensações</h4>{compensacaoFinanceira.pendencias.map((p) => <p key={p} className="text-amber-800">{p}</p>)}{compensacaoFinanceira.ajustes.map((a) => <p key={a.cobrancaId}>Cobrança {a.cobrancaId}: {a.apuracao.quantidadePendente} dias ainda devidos, ajuste {a.apuracao.valorAjusteApurado} {c.moeda}. Serviço após ajuste: {a.valorServicoAposCompensacao}; saldo: {a.saldoDevido}; crédito apurado: {a.creditoApurado}. Valores substituem os da mesma cobrança na conferência; não devem ser somados aos saldos anteriores.</p>)}</div>}
    {compensacaoFinanceira?.consolidado && <div className="rounded border p-2"><h4>Componente mensal após compensações</h4><p>Serviço: {compensacaoFinanceira.consolidado.totalServico} · Multa contratual: {compensacaoFinanceira.consolidado.multaContratual} · Saldo devido: {compensacaoFinanceira.consolidado.saldoDevidoSemCompensarCreditos} · Crédito separado: {compensacaoFinanceira.consolidado.creditoApuradoSemUtilizacao} {c.moeda}.</p><p>Este subtotal substitui o componente mensal anterior. O acerto completo ainda depende dos demais ajustes e da aprovação.</p></div>}
    ...
    {excecao && <div className="rounded border border-amber-400 p-2"><p>Exceção de multa proposta, ainda sem aprovação: {excecao.tipo === "DISPENSAR" ? "dispensa" : "alteração"}.</p><p>Multa contratual: {excecao.valorContratual}; proposta: {excecao.valorProposto}. Saldo proposto antes das compensações: {excecao.saldoPropostoSemCompensarCreditos} {c.moeda}.</p>{excecao.saldoPropostoAposCompensacao != null && <p>Saldo proposto após compensações: {excecao.saldoPropostoAposCompensacao} {c.moeda}, mantendo o crédito separado.</p>}<p>Motivo: {excecao.motivo}. Exige aprovação independente específica.</p></div>}
  </div>)}</div>;
}
...
function ResumoConcluido({ snapshot }: { snapshot: unknown }) {
  ...
  return <div className="space-y-2">{r.data.contratos.map(c => <div key={c.calculo.matriculaId} className="rounded border p-3">
    ...
    {c.consolidacao?.totais && <><p>Saldo devido no acerto: {c.consolidacao.totais.saldoDevidoSemCompensarCreditos}.</p>
  ...
  return <section className="space-y-3 border-t pt-3">
    ...
    {contextos && <form key={geracao} className="space-y-4" onChange={limparPrevia} onSubmit={(e) => {
      ...
      {contextos.map((c) => <div key={c.matriculaId} className="space-y-2 rounded border p-3">
        ...
        {c.ajustes.length > 0 && <details><summary>Ajustes anteriores</summary>{c.ajustes.map((a) => <p key={a.id}>{a.tipo}: {a.valorDe} para {a.valorPara} {a.moeda}. {a.motivo}</p>)}</details>}
        ...
        {c.condicoes?.regras.multa.tipo === "SEM_PREVISAO" ? <p>Sem previsão de multa: {c.condicoes.regras.multa.motivo}</p> : c.condicoes && <div className="space-y-2">
          <p>Cláusula {c.condicoes.regras.multa.clausulaId}: {c.condicoes.regras.multa.condicoesAplicacao}. {c.condicoes.regras.multa.tipo === "VALOR_FIXO" ? `Valor fixo: ${c.condicoes.regras.multa.valor} ${c.moeda}` : `${c.condicoes.regras.multa.percentual}% sobre ${c.condicoes.regras.multa.descricaoBase}`}</p>
