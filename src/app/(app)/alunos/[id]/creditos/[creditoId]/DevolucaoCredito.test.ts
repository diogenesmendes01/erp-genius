...
it("aciona execução, cancelamento e conciliação pelos callbacks", async () => {
  ...
  mocks.useState.mockReturnValue(["", vi.fn()]); mocks.useTransition.mockReturnValue([false, (cb: () => void) => cb()]); mocks.useRef.mockReturnValueOnce({ current: "" }).mockReturnValueOnce({ current: new Map() }); mocks.executar.mockResolvedValue({ ok: false, erro: "rede" }); mocks.cancelar.mockResolvedValue({ ok: true }); vi.stubGlobal("crypto", { randomUUID: () => "k" }); vi.stubGlobal("FormData", class { get(n: string) { return ({ resultado: "INCERTO", referencia: "ref", evidencia: "evidencia válida", motivo: "motivo válido" } as Record<string, string>)[n] ?? null; } });
  const forms = todos(DevolucaoCredito({ dados: { ...dados, devolucoes: [base] } }), "form"); await (forms[1].props!.onSubmit as any)({ preventDefault: vi.fn(), currentTarget: {} }); await Promise.resolve(); await (forms[2].props!.onSubmit as any)({ preventDefault: vi.fn(), currentTarget: {} }); await Promise.resolve(); expect(mocks.cancelar).toHaveBeenCalled(); expect(mocks.executar).toHaveBeenCalledWith(expect.objectContaining({ chaveIdempotencia: "k" }));
  const incerto = { ...base, podeExecutar: false, podeCancelar: false, podeConciliar: true, decisao: { ...base.decisao, reserva: { ...base.decisao.reserva, estado: "INCERTO" } } }; mocks.useRef.mockReset().mockReturnValue({ current: "" }); await (todos(DevolucaoCredito({ dados: { ...dados, devolucoes: [incerto] } }), "form")[1].props!.onSubmit as any)({ preventDefault: vi.fn(), currentTarget: {} }); await Promise.resolve(); expect(mocks.conciliar).toHaveBeenCalled();
});
