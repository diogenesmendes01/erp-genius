  ...
  return <div className="space-y-4"><Link className="underline" href={`/alunos/${id}`}>Voltar à ficha do aluno</Link>
    ...
    {!r.ok && <p role="alert">{r.erro}</p>}{r.ok && r.dado && <>
      {r.dado.origemDesistencia && <section className="rounded border p-4 space-y-2" aria-label="Origem do crédito">
        ...
        <p>Valor original: {r.dado.moeda} {r.dado.origemDesistencia.valorOriginal}. O crédito foi apurado em acerto aprovado; isso não comprova devolução de dinheiro.</p>
