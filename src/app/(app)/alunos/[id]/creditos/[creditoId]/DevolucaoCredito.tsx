...
export function DevolucaoCredito({ dados }: { dados: Dados }) {
  ...
  return <section className="space-y-4 border-t pt-4"><h2 className="text-xl font-medium">Devolução de crédito</h2><p>Disponível: {dados.valorCredito} {dados.moeda} · reservado ou devolvido: {dados.reservaDevolucao} · saída confirmada: {dados.devolvido}. Registrar a operação manualmente não cria recebimento. A aprovação reserva o valor; uma resposta incerta só é encerrada por conciliação com evidência.</p>
    ...
    <form onChange={() => { chave.current = ""; }} onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); chave.current ||= crypto.randomUUID(); const k = chave.current; enviar(async () => { const r = await proporDevolucaoCredito({ creditoId: dados.creditoId, valor: String(f.get("valor")), pedidoAluno: String(f.get("pedidoAluno")), evidenciaPedido: String(f.get("evidenciaPedido")), destino: String(f.get("destino")), motivo: String(f.get("motivo")), chaveIdempotencia: k }); if (r.ok) chave.current = ""; return r; }); }}><fieldset disabled={ocupado} className="space-y-2"><legend>Nova proposta de devolução</legend>
      ...
      <label className="block">Motivo<textarea className={estilo} name="motivo" minLength={5} maxLength={2000} required /></label><button className={estilo}>Guardar proposta</button>
    </fieldset></form>
    {dados.devolucoes.map(p => <article key={p.id} className="space-y-2 rounded border p-3"><h3>Devolução {p.versao} · {p.valor} {dados.moeda}</h3><p>Destino: {p.destino}</p><p>Pedido: {p.pedidoAluno}</p><p>Evidência: {p.evidenciaPedido}</p>
...
function Reserva({ dados: p, ocupado, enviar }: { dados: Dados["devolucoes"][number]; ocupado: boolean; enviar: (acao: () => Promise<{ ok: boolean; erro?: string }>) => void }) {
  ...
  return <div className="space-y-2 rounded bg-muted p-2"><p>Reserva: {r.estado}{r.referenciaExterna ? ` · referência ${r.referenciaExterna}` : ""}</p>{r.evidenciaExecucao && <p>Comprovante: {r.evidenciaExecucao}</p>}
    ...
    {p.podeCancelar && r.estado === "AGUARDANDO_EXECUCAO" && <form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); enviar(() => cancelarDevolucaoCredito({ reservaId: r.id, motivo: String(f.get("motivo")), evidenciaCancelamento: String(f.get("evidencia")) })); }}><fieldset disabled={ocupado} className="space-y-2"><label className="block">Cancelar reserva<input className={estilo} name="motivo" minLength={5} required placeholder="Motivo" /></label><label className="block">Evidência<textarea className={estilo} name="evidencia" minLength={5} required /></label><button className={estilo}>Cancelar e liberar reserva</button></fieldset></form>}
    {p.podeExecutar && r.estado === "AGUARDANDO_EXECUCAO" && <form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); enviar(() => registrarExecucaoDevolucaoCredito({ reservaId: r.id, resultado: String(f.get("resultado")) as "CONFIRMADA" | "INCERTO", referenciaExterna: String(f.get("referencia")), evidenciaExecucao: String(f.get("evidencia")), chaveIdempotencia: crypto.randomUUID() })); }}><fieldset disabled={ocupado} className="space-y-2"><label className="block">Resultado<select className={estilo} name="resultado"><option value="CONFIRMADA">Saída confirmada manualmente</option><option value="INCERTO">Resposta externa incerta</option></select></label><label className="block">Referência externa<input className={estilo} name="referencia" minLength={1} required /></label><label className="block">Comprovante<textarea className={estilo} name="evidencia" minLength={5} required /></label><button className={estilo}>Registrar execução</button></fieldset></form>}
    {p.podeExecutar && r.estado === "INCERTO" && <form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); enviar(() => conciliarDevolucaoCredito({ reservaId: r.id, confirmouSaida: f.get("resultado") === "confirmar", evidenciaConciliacao: String(f.get("evidencia")) })); }}><fieldset disabled={ocupado} className="space-y-2"><label className="block">Conciliação<select className={estilo} name="resultado"><option value="confirmar">Confirmar saída</option><option value="liberar">Liberar reserva</option></select></label><label className="block">Evidência<textarea className={estilo} name="evidencia" minLength={5} required /></label><button className={estilo}>Conciliar resultado</button></fieldset></form>}
  </div>;
}
