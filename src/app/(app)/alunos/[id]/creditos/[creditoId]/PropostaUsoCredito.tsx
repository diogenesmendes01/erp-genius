  ...
  return <div className="space-y-4"><p>Crédito disponível após utilizações aprovadas: {dados.valorCredito} {dados.moeda}.</p>
    ...
    {!dados.cobrancas.length ? <p>Nenhuma cobrança em aberto disponível nesta matrícula.</p> : <form onChange={() => { chave.current = ""; }} onSubmit={e => {
      ...
      iniciar(async () => { setErro(""); try {
        const r = await proporUtilizacaoCredito({ creditoId: dados.creditoId, cobrancaId: String(f.get("cobranca")), valor: String(f.get("valor")), concordancia: String(f.get("concordancia")), motivo: String(f.get("motivo")), chaveIdempotencia });
        if (!r.ok) { setErro(r.erro); return; } chave.current = ""; router.refresh();
      ...
      <label className="block">Valor a utilizar<input type="number" name="valor" required min="0.01" step="0.01" max={dados.valorCredito} className={estilo} /></label>
