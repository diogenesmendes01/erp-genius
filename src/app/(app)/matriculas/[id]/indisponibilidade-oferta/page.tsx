...
export default async function Page({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ pagina?: string }> }) {
  ...
  return <div className="space-y-4">
    ...
    {!resultado.ok || !resultado.dado ? <p role="alert">{resultado.ok ? "Histórico indisponível." : resultado.erro}</p> : <RelatosIndisponibilidadeOferta matriculaId={id} dados={resultado.dado} fusoInstitucional={fuso.success ? fuso.data : null} fusoExibicao={fusoExibicao} />}
  </div>;
}
