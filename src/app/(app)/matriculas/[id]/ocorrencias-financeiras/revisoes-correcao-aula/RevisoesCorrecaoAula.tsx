...
const Foto = z.object({ fundamento: z.object({ participacaoAnterior: z.string(), participacaoProposta: z.string(), ocorrenciaHistoricaTipo: z.string(), desfechoHistorico: z.string(), minutosEquivalentes: z.number(), valorPreservado: z.union([z.number(), z.string()]), moeda: z.string() }), ocorrencia: z.object({ id: z.string(), versao: z.number() }), conferencia: z.object({ id: z.string() }), cobranca: z.object({ id: z.string(), status: z.string(), valorLiquidadoCredito: z.union([z.number(), z.string()]), valorCompensadoPermuta: z.union([z.number(), z.string()]) }).nullable(), informesPagamento: z.array(z.object({ id: z.string(), status: z.string() })) }).passthrough();
...
export function RevisoesCorrecaoAula({ matriculaId, dados }: { matriculaId: string; dados: Dados }) {
  ...
  return <section className="space-y-4">
    ...
    {dados.revisoes.map(r => { const foto = Foto.safeParse(r.fotografia), podeDecidir = r.preparador.id !== dados.usuarioId; return <article key={r.id} className="rounded border p-4 space-y-2"><p>Q23 v{r.propostaCorrecaoAula.versao} · revisão financeira v{r.versao} · {data(r.propostaCorrecaoAula.encontro.inicio, r.propostaCorrecaoAula.encontro.fusoOrigem)}</p><p>Preparada por {r.preparador.nome}: {r.motivo}</p>{foto.success ? <section className="text-sm"><p>Fundamento Q92: {foto.data.fundamento.participacaoAnterior} → {foto.data.fundamento.participacaoProposta}; ocorrência v{foto.data.ocorrencia.versao} {foto.data.fundamento.ocorrenciaHistoricaTipo}, conferência {foto.data.fundamento.desfechoHistorico}.</p><p>{foto.data.fundamento.minutosEquivalentes} minutos · valor preservado {foto.data.fundamento.valorPreservado} {foto.data.fundamento.moeda}.</p><p>{foto.data.cobranca ? `Cobrança ${foto.data.cobranca.status}; crédito liquidado ${foto.data.cobranca.valorLiquidadoCredito}; permuta ${foto.data.cobranca.valorCompensadoPermuta}.` : "Ainda sem item de emissão."}</p><p>Informes: {foto.data.informesPagamento.length ? foto.data.informesPagamento.map(i => i.status).join(", ") : "nenhum"}.</p></section> : <p role="alert">Fotografia histórica indisponível para apresentação; não decida sem nova conferência.</p>}{r.decisao ? <p>Decisão: {r.decisao.aprovada ? "aprovada" : "rejeitada"} por {r.decisao.decisor.nome}. {r.decisao.motivo}</p> : podeDecidir ? <form onSubmit={e => { e.preventDefault(); decidir(r.id, true, e.currentTarget); }} className="space-y-2"><label className="block">Motivo da decisão<textarea name="motivo" required minLength={5} maxLength={3000} disabled={ocupado} className="block w-full rounded border p-2" /></label><button disabled={ocupado} className="rounded border p-2">Aprovar sem alteração de valores</button><button disabled={ocupado} formAction="#" onClick={e => { e.preventDefault(); const form = e.currentTarget.form; if (form) decidir(r.id, false, form); }} className="ml-2 rounded border p-2">Rejeitar</button></form> : <p>Outra pessoa com alçada financeira deve decidir esta revisão.</p>}</article>; })}
