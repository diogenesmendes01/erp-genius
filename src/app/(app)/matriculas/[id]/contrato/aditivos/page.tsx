...
export default async function AditivosPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ pagina?: string; modelos?: string }> }) {
  await exigirSessaoPagina(Papel.SECRETARIA_ACADEMICA); const { id } = await params, s = await searchParams;
  const r = await consultarAditivosContratuais({ matriculaId: id, pagina: pagina(s.pagina), paginaModelos: pagina(s.modelos) });
  if (!r.ok || !r.dado) return <p role="alert">{r.ok ? "Consulta indisponível." : r.erro}</p>;
  const d = r.dado, base = `/matriculas/${encodeURIComponent(id)}/contrato/aditivos`;
  return <div className="space-y-5"><Link className="underline" href={`/matriculas/${encodeURIComponent(id)}/contrato`}>Voltar aos documentos</Link><h1 className="text-2xl">Aditivos contratuais · {d.matricula.aluno}</h1>
    ...
    {d.fonte ? <><a className="underline" href={`/api/matriculas/${encodeURIComponent(id)}/originais/${encodeURIComponent(d.fonte.artefatoId)}/pdf`} target="_blank" rel="noopener noreferrer">Abrir PDF do original preservado</a>{d.fonte.ambiente === "SANDBOX" && <p role="status">Ambiente de teste: a conclusão preservada é identificada como SANDBOX.</p>}{d.modelos.length ? <PrepararAditivo matriculaId={id} fonte={d.fonte} modelos={d.modelos} /> : <p>Nenhum modelo de aditivo aprovado está disponível nesta página.</p>}<nav className="flex gap-4" aria-label="Páginas dos modelos">{d.paginaModelos > 1 && <Link href={`${base}?modelos=${d.paginaModelos - 1}&pagina=${d.pagina}`}>Modelos anteriores</Link>}<span>Página {d.paginaModelos}</span>{d.maisModelos && <Link href={`${base}?modelos=${d.paginaModelos + 1}&pagina=${d.pagina}`}>Mais modelos</Link>}</nav></> : <p role="status">{d.impedimento}</p>}
