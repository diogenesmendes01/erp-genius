...
export default async function AditivosPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ pagina?: string; modelos?: string; agenda?: string }> }) {
  ...
  return <div className="space-y-5"><Link className="underline" href={`/matriculas/${encodeURIComponent(id)}/contrato`}>Voltar aos documentos</Link><h1 className="text-2xl">Aditivos contratuais · {d.matricula.aluno}</h1>
    ...
    {agendaResultado && (!agendaResultado.ok || !agenda) && <p role="alert">{agendaResultado.ok ? "Fotografia indisponível." : agendaResultado.erro}</p>}{d.fonte ? <>{d.fonte.tipo === "ORIGEM_HISTORICA" ? <><p role="status">Fonte contratual: origem histórica aprovada (contrato legado transcrito e conferido). Taxa, primeira mensalidade e adiantamento não admitem aditivo.</p><Link className="underline" href={`/matriculas/${encodeURIComponent(id)}/contrato/origem-historica`}>Ver origem histórica</Link></> : <a className="underline" href={`/api/matriculas/${encodeURIComponent(id)}/originais/${encodeURIComponent(d.fonte.artefatoId ?? "")}/pdf`} target="_blank" rel="noopener noreferrer">Abrir PDF do original preservado</a>}{d.fonte.ambiente === "SANDBOX" && <p role="status">Ambiente de teste: a conclusão preservada é identificada como SANDBOX.</p>}{d.modelos.length ? <PrepararAditivo matriculaId={id} fonte={d.fonte} modelos={d.modelos} agenda={agenda} /> : <p>Nenhum modelo de aditivo aprovado está disponível nesta página.</p>}<nav className="flex gap-4" aria-label="Páginas dos modelos">{d.paginaModelos > 1 && <Link href={`${base}?modelos=${d.paginaModelos - 1}&pagina=${d.pagina}`}>Modelos anteriores</Link>}<span>Página {d.paginaModelos}</span>{d.maisModelos && <Link href={`${base}?modelos=${d.paginaModelos + 1}&pagina=${d.pagina}`}>Mais modelos</Link>}</nav></> : <p role="status">{d.impedimento} <Link className="underline" href={`/matriculas/${encodeURIComponent(id)}/contrato/origem-historica`}>Contrato legado? Registrar origem histórica.</Link></p>}
