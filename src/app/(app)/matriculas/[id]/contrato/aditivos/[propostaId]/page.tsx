...
export default async function PropostaAditivoPage({ params, searchParams }: { params: Promise<{ id: string; propostaId: string }>; searchParams: Promise<{ paginaConferencias?: string; paginaOriginais?: string }> }) {
  ...
  const r = await consultarPropostaAditivo({ matriculaId: id, propostaId }); if (!r.ok || !r.dado) return <p role="alert">{r.ok ? "Consulta indisponível." : r.erro}</p>;
  const d = r.dado, data = d.criadaEm.toISOString().replace("T", " ").slice(0, 19) + " UTC";
  ...
  return <div className="space-y-5"><Link className="underline" href={`/matriculas/${encodeURIComponent(id)}/contrato/aditivos`}>Voltar às propostas</Link><h1 className="text-2xl">Proposta de aditivo · versão {d.versao}</h1><p>Preparada por {d.preparadaPor} em {data}.</p><p className="whitespace-pre-wrap">{d.motivo}</p>{d.ambiente === "SANDBOX" && <p role="status">Ambiente de teste: esta fonte não comprova formalização em produção.</p>}
    ...
    <ImpactosPainel classificacao={d.impactos} /><p>Vigência proposta: {d.vigenciaInicio.toISOString().replace("T", " ").slice(0, 19)} UTC.</p>
    {acertoResultado.ok && acertoResultado.dado?.estado === "PRONTA_PARA_SELECAO" && <AcertoTaxaFormulario matriculaId={id} propostaAditivoId={propostaId} conclusaoId={acertoResultado.dado.conclusaoId} revisaoHash={acertoResultado.dado.revisaoHash} cobrancas={acertoResultado.dado.cobrancas} />}
    <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Revisão dos efeitos</h2>{!efeitos ? <p role="alert">{efeitosResultado.ok ? "Revisão de efeitos indisponível." : efeitosResultado.erro}</p> : <div className="space-y-3"><p>Esta revisão descreve os efeitos previstos para a vigência indicada. A consulta não altera o cadastro global do aluno; o registro de aplicação aparece abaixo.</p><p>Vigência prevista: {efeitos.vigenciaInicio.toISOString().replace("T", " ").slice(0, 19)} UTC.</p>{efeitos.primeiraMensalidade && <section className="rounded border p-3"><h3 className="font-medium">Vencimento da primeira mensalidade</h3><p>Proposto: {efeitos.primeiraMensalidade.vencimentoProposto}.</p>{efeitos.primeiraMensalidade.cobranca && <p>Cobrança de origem: {efeitos.primeiraMensalidade.cobranca.id} · versão {efeitos.primeiraMensalidade.cobranca.versao}.</p>}<p role="status">{efeitos.primeiraMensalidade.pendencia}</p></section>}<ul className="space-y-2">{efeitos.efeitos.map(efeito => { const alteracao = d.alteracoes.find(a => a.campo === efeito.origem); return <li className="rounded border p-3" key={`${efeito.origem}:${efeito.dominio}:${efeito.destino}`}><p className="font-medium">{alteracao?.rotulo ?? efeito.origem}</p><p>{dominios[efeito.dominio] ?? "Condições deste contrato"}.</p><p>Novo valor: {alteracao?.novo ?? "Valor estruturado preservado para revisão."}</p>{efeito.exigeAcerto && <p role="status">Exige acerto próprio antes de qualquer aplicação.</p>}</li>; })}</ul>{efeitos.pendencias.length > 0 && <section><h3 className="font-medium">Pendências</h3><ul className="list-disc pl-5">{efeitos.pendencias.map(pendencia => <li key={`${pendencia.origem}:${pendencia.mensagem}`}>{d.alteracoes.find(a => a.campo === pendencia.origem)?.rotulo ?? pendencia.origem}: {pendencia.mensagem}</li>)}</ul></section>}<p role="status">{efeitos.aplicado ? "Esta versão possui aplicação registrada. A vigência aprovada e os acertos financeiros próprios continuam sendo respeitados." : "Esta proposta ainda não possui aplicação das condições registrada. Consultar esta revisão não aplica efeitos."}</p></div>}</section>
    ...
    {d.superada && <p role="status">Existe uma proposta mais recente. Esta versão pode ser consultada ou rejeitada, mas não aprovada.</p>}{d.decisao ? <section className="rounded border p-4"><h2 className="text-xl">{d.decisao.aprovada ? "Proposta aprovada" : "Proposta rejeitada"}</h2><p>{d.decisao.decisor.nome} · {d.decisao.decididaEm.toISOString().replace("T", " ").slice(0, 19)} UTC</p><p className="whitespace-pre-wrap">{d.decisao.motivo}</p></section> : d.podeDecidir ? <DecidirAditivo propostaId={d.id} propostaHash={d.propostaHash} superada={d.superada} /> : <p>A decisão exige outra pessoa da Administração.</p>}
    {d.decisao?.aprovada && !d.superada && <ParticipantesFormulario matriculaId={id} propostaId={propostaId} />}
    <ParticipantesHistorico matriculaId={id} propostaId={propostaId} pagina={pagina} />
    <OriginaisPainel matriculaId={id} propostaId={propostaId} pagina={paginaOriginais} podeGerar={Boolean(d.decisao?.aprovada && !d.superada)} />
