...
import { consultarAplicacaoCondicoesAditivo, consultarCondicoesAditivo } from "@/server/contratos/aditivo-condicoes";
...
import { EnvioAditivoFormulario } from "../../../EnvioAditivoFormulario";
...
export default async function ConferenciaOriginalAditivoPage({ params, searchParams }: {
  ...
  searchParams: Promise<{ pagina?: string; paginaTentativas?: string; tentativaObservacoes?: string; paginaObservacoes?: string }>;
}) {
  await exigirSessaoPagina(Papel.SECRETARIA_ACADEMICA);
  const { id, propostaId, artefatoId } = await params, parametros = await searchParams, numero = Number(parametros.pagina ?? 1), numeroTentativas = Number(parametros.paginaTentativas ?? 1), numeroTentativaObservacoes = Number(parametros.tentativaObservacoes ?? 0), numeroPaginaObservacoes = Number(parametros.paginaObservacoes ?? 1);
  ...
  const tentativaObservacoes = Number.isInteger(numeroTentativaObservacoes) && numeroTentativaObservacoes >= 1 && numeroTentativaObservacoes <= 100000 ? numeroTentativaObservacoes : undefined;
  const paginaObservacoes = Number.isInteger(numeroPaginaObservacoes) && numeroPaginaObservacoes >= 1 && numeroPaginaObservacoes <= 100000 ? numeroPaginaObservacoes : 1;
  const [r, preferencia] = await Promise.all([consultarAssinaturaAditivo({ matriculaId: id, propostaId, artefatoId, pagina }), consultarPreferenciaFusoEquipe()]);
  ...
  const processoResultado = await consultarProcessoAssinaturaAditivo({ matriculaId: id, propostaId, artefatoId, paginaTentativas, tentativaObservacoes, paginaObservacoes });
  ...
  const integracaoResultado = await consultarIntegracaoAssinatura();
  // O envio integrado só aparece quando o driver ativo é o mesmo fornecedor/ambiente do processo preparado.
  const integrado = integracaoResultado.ok && !!integracaoResultado.dado && !!processo && integracaoResultado.dado.fornecedor === processo.fornecedor && integracaoResultado.dado.ambiente === processo.ambiente;
  ...
  const aplicacaoHistoricoResultado = await consultarAplicacaoCondicoesAditivo({ matriculaId: id, propostaId });
  ...
  return <div className="space-y-4"><Link className="underline" href={base}>Voltar ao aditivo</Link>
    ...
    {d.revisao && <><section className="space-y-2 rounded border p-4"><h2 className="text-xl">Documento e participantes</h2>
      ...
      <p>Vigência: {vigencia(d.revisao.dados.vigenciaInicio)}.</p>
    ...
    {processo ? <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Processo de assinatura preparado</h2><p>Fornecedor: {processo.fornecedor} · ambiente: {processo.ambiente}.</p><p>Estado: {processo.estado}. Tentativa atual: {processo.tentativaAtual}.</p><p>Preparado em {data(processo.criadoEm)}.</p>{processo.referenciaExterna && <p>Referência externa registrada: {processo.referenciaExterna}.</p>}
      {integrado && processo.estado === "PREPARADO" && conferenciaAtual && <EnvioAditivoFormulario modo="ENVIAR" matriculaId={id} propostaId={propostaId} processoId={processo.id} />}
      {integrado && (processo.estado === "ENVIANDO" || processo.estado === "ENVIO_INCERTO") && <EnvioAditivoFormulario modo="CONCILIAR" matriculaId={id} propostaId={propostaId} processoId={processo.id} />}
      <h3 className="font-medium">Histórico de tentativas</h3>{processo.tentativas.length ? <><ul className="space-y-2">{processo.tentativas.map(t => <li className="rounded border p-2" key={t.numero}>Tentativa {t.numero} · {data(t.iniciadaEm)}{t.observacoes.map(o => <p key={o.id}>Resultado: {o.resultado} em {data(o.observadaEm)}{o.referenciaExterna ? ` · referência ${o.referenciaExterna}` : ""}</p>)}{(t.paginaObservacoes > 1 || t.temMaisObservacoes) && <nav className="flex gap-3" aria-label={`Páginas das observações da tentativa ${t.numero}`}>{t.paginaObservacoes > 1 && <Link className="underline" href={`${atual}?pagina=${pagina}&paginaTentativas=${processo.paginaTentativas}&tentativaObservacoes=${t.numero}&paginaObservacoes=${t.paginaObservacoes - 1}`}>Observações mais recentes</Link>}<span>Página {t.paginaObservacoes}</span>{t.temMaisObservacoes && <Link className="underline" href={`${atual}?pagina=${pagina}&paginaTentativas=${processo.paginaTentativas}&tentativaObservacoes=${t.numero}&paginaObservacoes=${t.paginaObservacoes + 1}`}>Observações anteriores</Link>}</nav>}</li>)}</ul><nav aria-label="Páginas de tentativas" className="flex gap-3">{processo.paginaTentativas > 1 && <Link className="underline" href={`${atual}?pagina=${pagina}&paginaTentativas=${processo.paginaTentativas - 1}`}>Tentativas mais recentes</Link>}<span>Página {processo.paginaTentativas}</span>{processo.temMaisTentativas && <Link className="underline" href={`${atual}?pagina=${pagina}&paginaTentativas=${processo.paginaTentativas + 1}`}>Tentativas anteriores</Link>}</nav></> : <p role="status">Nenhuma tentativa de envio foi iniciada.</p>}<p role="status">Criar o processo não envia o documento e não registra assinaturas.</p></section> : conferenciaAtual && (ambienteFonte === "SANDBOX" || ambienteFonte === "PRODUCAO") ? <ProcessoFormulario matriculaId={id} propostaId={propostaId} artefatoId={artefatoId} conferenciaId={conferenciaAtual.id} ambiente={ambienteFonte} /> : <p role="status">Registre uma conferência correspondente à revisão atual antes de preparar o processo.</p>}
    {conclusao && <section className="space-y-2 rounded border p-4"><h2 className="text-xl">Conclusão de assinatura preservada</h2><p>Fornecedor: {conclusao.fornecedor} · ambiente: {conclusao.ambiente}.</p><p>Concluída em {data(conclusao.concluidaEm)}.</p><p>Assinaturas preservadas: {Array.isArray(conclusao.assinaturas) ? conclusao.assinaturas.length : 0}.</p><a className="block underline" target="_blank" rel="noopener noreferrer" href={`/api/matriculas/${encodeURIComponent(id)}/aditivos/${encodeURIComponent(propostaId)}/assinaturas/${encodeURIComponent(conclusao.id)}/pdf`}>Abrir PDF assinado preservado</a><a className="block underline" href={`/api/matriculas/${encodeURIComponent(id)}/aditivos/${encodeURIComponent(propostaId)}/assinaturas/${encodeURIComponent(conclusao.id)}/evidencias`}>Baixar evidências preservadas</a><p role="status">A consulta preserva os arquivos recebidos; ela não confirma aplicação das condições.</p></section>}
    ...
    {aplicacaoHistorica && <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Condições aplicadas</h2><p>Versão {aplicacaoHistoricoResultado.dado?.versao} aplicada em {data(aplicacaoHistorica.aplicadaEm)}.</p><p role="status">Registro histórico preservado. As cobranças já emitidas foram preservadas; acertos e agenda continuam nos fluxos próprios.</p></section>}
