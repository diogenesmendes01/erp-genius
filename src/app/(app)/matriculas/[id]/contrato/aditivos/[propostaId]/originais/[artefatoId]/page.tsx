...
export default async function ConferenciaOriginalAditivoPage({ params, searchParams }: {
  ...
  return <div className="space-y-4"><Link className="underline" href={base}>Voltar ao aditivo</Link>
    ...
    {conclusao?.ambiente === "PRODUCAO" && conferenciaFinal?.revisao && conferenciaFinal.historico && <section className="space-y-3 rounded border p-4">{condicoesDaProposta?.aplicacao ? <><h2 className="text-xl">Condições aplicadas</h2><p>Versão {condicoesDaProposta.versao} aplicada em {condicoesDaProposta.aplicacao.aplicadaEm.toISOString().replace("T", " ").slice(0, 19)} UTC.</p><p role="status">As cobranças já emitidas foram preservadas. Acertos e agenda continuam nos fluxos próprios.</p></> : <CondicoesFormalizadasFormulario matriculaId={id} propostaId={propostaId} conclusaoId={conclusao.id} revisaoHash={conferenciaFinal.revisao.hash} formalizada={Boolean(condicoesDaProposta)} />}</section>}
