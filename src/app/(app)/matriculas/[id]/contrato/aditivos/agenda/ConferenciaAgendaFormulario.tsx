...
export function ConferenciaAgendaFormulario({ matriculaId, encontros, professores }: { matriculaId: string; encontros: Encontro[]; professores: Professor[] }) {
  ...
  return <section className="space-y-4 rounded border p-4"><div><h2 className="text-xl">Conferência de agenda</h2><p>Conferência — não reserva nem altera agenda.</p></div>
    {!encontros.length ? <EstadoVazio role="status">Não há encontros particulares futuros disponíveis para esta matrícula.</EstadoVazio> : <form className="space-y-4" onChange={() => setResultado(null)} onSubmit={evento => { evento.preventDefault(); setMensagem(""); setResultado(null); const dados = new FormData(evento.currentTarget); try { const valores = Object.fromEntries(selecionados.map(encontroId => [encontroId, { professorNovoId: String(dados.get(`professor-${encontroId}`) ?? ""), fusoOrigem: String(dados.get(`fuso-${encontroId}`) ?? ""), inicioLocal: String(dados.get(`inicio-${encontroId}`) ?? ""), fimLocal: String(dados.get(`fim-${encontroId}`) ?? "") }])); const alteracoes = montarAlteracoesAgenda(selecionados, valores); if (!alteracoes.length) { setMensagem("Selecione pelo menos um encontro para conferir."); return; } iniciar(async () => { const r = await consultarConferenciaAgendaAditivo({ matriculaId, encontros: alteracoes }); if (!r.ok || !r.dado) setMensagem(r.ok ? "Conferência indisponível." : r.erro); else setResultado(r.dado); }); } catch { setMensagem("Informe datas, horários e fuso válidos."); } }}>
      ...
      {mensagem && <p role="alert">{mensagem}</p>}<button className={botaoClasses({ variante: "secundario", tamanho: "lg" })} disabled={pendente}>{pendente ? "Conferindo…" : "Conferir alterações"}</button>
    </form>}
