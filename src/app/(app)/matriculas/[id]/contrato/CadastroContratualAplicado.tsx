...
export function CadastroContratualAplicado({ cadastro, preferenciaFusoExibicao = null }: { cadastro: {
  versao: number; vigenciaInicio: string; estado: "PROGRAMADO" | "VIGENCIA_INICIADA";
  campos: Partial<Record<string, string>>;
} | null | undefined; preferenciaFusoExibicao?: string | null }) {
  if (!cadastro) return null;
  const campos = Object.entries(cadastro.campos).filter(([campo, valor]) => rotulos[campo] && valor !== undefined);
  if (!campos.length) return null;
  const vigencia = formatarInstanteExibicao(cadastro.vigenciaInicio, preferenciaFusoExibicao, "UTC");
  return <section className="space-y-2 rounded border p-4" aria-label="Cadastro contratual da versão aplicada">
    ...
    <p>{cadastro.estado === "PROGRAMADO" ? "Vigência programada" : "Vigência iniciada"}: {vigencia.texto} (horário exibido em {vigencia.fuso}; referência contratual preservada).</p>
