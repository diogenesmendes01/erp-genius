...
export default async function PropostaPage({ params, searchParams }: { params: Promise<{ id: string; propostaId: string }>; searchParams: Promise<{ retornos?: string }> }) {
  ...
  const r = await consultarPropostaSubstituicao({ matriculaId: id, propostaId, paginaObservacoes });
  ...
  return <div className="space-y-5">
    ...
    <p>Preparada por {d.preparadaPor} em {d.criadaEm.toISOString().replace("T", " ").slice(0, 19)} UTC.</p>
    <p className="whitespace-pre-wrap">{d.motivo}</p>
    <AndamentoSubstituicao dados={d.andamento} matriculaId={id} propostaId={propostaId} />
    ...
    <div className="grid gap-5 xl:grid-cols-2">{[{ titulo: "Original enviado", ...d.fonte }, { titulo: "Documento substituto", ...d.substituto }].map(doc => <section key={doc.artefatoId} className="space-y-3">
      ...
      <section className="space-y-2 rounded border p-3"><h3 className="font-medium">Pessoas identificadas para assinatura</h3>
        ...
        <p>Reserva na revisão: {doc.assinatura.reserva.status.replaceAll("_", " ")} · prazo {doc.assinatura.reserva.expiraEm.replace("T", " ").replace("Z", " UTC")}.</p>
      </section>
    </section>)}</div>
    {d.decisao ? <section className="rounded border p-4"><h2 className="text-xl">{d.decisao.aprovada ? "Proposta aprovada" : "Proposta rejeitada"}</h2><p>{d.decisao.decisor.nome} · {d.decisao.decididaEm.toISOString().replace("T", " ").slice(0, 19)} UTC</p><p className="whitespace-pre-wrap">{d.decisao.motivo}</p></section>
