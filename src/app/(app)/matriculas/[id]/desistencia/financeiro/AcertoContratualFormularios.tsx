...
export function PrepararAcertoContratualFormulario({ pedidoId, condicoesId }: { pedidoId: string; condicoesId: string }) {
  const op = useOperacao();
  return <form className="space-y-3 rounded border p-4" onSubmit={(e: FormEvent<HTMLFormElement>) => { e.preventDefault(); const f = new FormData(e.currentTarget); void op.executar(() => prepararAcertoDesistenciaContratual({ pedidoId, condicoesId, motivo: String(f.get("motivo") ?? ""), chaveIdempotencia: op.chave.current }), "Memória contratual preparada. Outra pessoa autorizada deve decidir."); }}>
    ...
    <fieldset disabled={op.ocupado}><label className="block">Motivo<textarea name="motivo" required minLength={5} maxLength={3000} className="mt-1 block w-full rounded border p-2" /></label><button className="mt-3 rounded border px-3 py-2">{op.ocupado ? "Preparando…" : "Preparar memória contratual"}</button></fieldset>{op.mensagem && <p role="status">{op.mensagem}</p>}
  </form>;
}
