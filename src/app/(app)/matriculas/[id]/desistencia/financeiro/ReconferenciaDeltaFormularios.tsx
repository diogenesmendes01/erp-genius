...
export function PrepararReconferenciaDeltaFormulario({ aplicacaoBaseId }: { aplicacaoBaseId: string }) {
  ...
  return <form className="space-y-3 rounded border p-4" onSubmit={(event: FormEvent<HTMLFormElement>) => {
    ...
    <fieldset disabled={op.ocupado || op.incerta}><label className="block">Motivo<textarea name="motivo" required minLength={5} maxLength={3000} className="mt-1 block w-full rounded border p-2" /></label><button className="mt-3 rounded border px-3 py-2">{op.ocupado ? "Preparando…" : "Preparar reconferência"}</button></fieldset><ReconciliarTentativaDelta op={op} />{op.mensagem && <p role="status">{op.mensagem}</p>}
  </form>;
}
