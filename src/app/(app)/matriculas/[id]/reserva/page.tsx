...
export default async function ReservaContratacaoPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ pagina?: string }> }) {
  ...
  return <div className="space-y-4"><Link className="underline" href={`/secretaria?matriculaId=${id}`}>Voltar à contratação</Link><h1 className="text-2xl font-medium">Reserva de vaga · {r.matricula.codigo ?? "Contratação em preparação"}</h1>
    ...
    {r.reservas.map((v) => { const prazo = formatarInstanteExibicao(v.expiraEm, fusoExibicao, v.janela.fusoAdmissao); return <section key={v.id} className="rounded border p-4"><p role="status">{v.status === "MANTIDA_PENDENCIA" ? "Vaga mantida por pendência" : "Reserva ativa"} · {v.turma.codigo ?? v.turma.nome}</p><p>Prazo: {prazo.texto} · {prazo.fuso}</p><p>A Secretaria acompanha a reserva e eventuais pendências.</p></section>; })}
