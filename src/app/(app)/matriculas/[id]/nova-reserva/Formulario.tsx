...
export function NovaReservaFormulario({ base }: { base: Base }) {
  ...
  return <div className="space-y-4"><fieldset disabled={ocupado} className="space-y-3">
    ...
    {revisao && <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Conferência da retomada</h2>
      ...
      <h3>Cobranças existentes — não serão reemitidas</h3><ul>{revisao.cobrancas.map((c) => <li key={c.id}>{c.tipo} · {c.moeda} {c.valorNegociado} · saldo {c.saldo ?? "a conferir"} · {c.status} · {c.informesPendentes} comprovante(s) pendente(s)</li>)}</ul>
      <h3>Condições de entrada registradas</h3><ul>{revisao.plano.map((p) => <li key={p.tipo}>{p.tipo} · {p.moeda} {p.valor} · vencimento {p.vencimento}{p.cobertura ? ` · cobertura ${p.cobertura.inicio} a ${p.cobertura.fim}` : ""}</li>)}</ul>
