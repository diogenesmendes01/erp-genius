...
export default async function NovaGradePage({ searchParams }: { searchParams: Promise<{ busca?: string; cursor?: string }> }) {
  await exigirSessaoPagina(Papel.SECRETARIA_ACADEMICA, Papel.GERENTE_PEDAGOGICO);
  const q = await searchParams, busca = (q.busca ?? "").trim().slice(0, 100);
  const turmas = await prisma.turma.findMany({
    where: { status: "PLANEJADA", ...(busca ? { codigo: { contains: busca, mode: "insensitive" as const } } : {}), aulasDiario: { none: {} }, encontrosAgenda: { none: { status: { not: "RASCUNHO" } } } },
  ...
  return <div className="space-y-5">
    ...
    <form className="flex flex-wrap items-end gap-3"><label>Buscar pelo código<input name="busca" defaultValue={busca} maxLength={100} className="ml-2 rounded border bg-[var(--surface)] p-2" /></label><button className="rounded border px-3 py-2">Buscar turmas</button></form>
    {!turmas.length ? <p>Nenhuma turma planejada sem histórico ou agenda publicada foi encontrada.</p> : <PrepararGrade key={busca + (q.cursor ?? "")} turmas={turmas.slice(0, 30).map((t) => ({ id: t.id, codigo: t.codigo ?? `Turma sem código (${t.id})`, versao: t.propostasGrade[0]?.versao ?? 0,
