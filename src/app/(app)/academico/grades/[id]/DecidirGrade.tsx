...
export function DecidirGrade({ id, podePublicar }: { id: string; podePublicar: boolean }) {
  ...
  return <section className="space-y-3 border-t pt-4">
    ...
    <div className="flex flex-wrap gap-3">
      <button className="rounded bg-brand-solid px-3 py-2 text-white disabled:opacity-50" disabled={ocupado || !podePublicar || motivo.trim().length < 5} onClick={() => decidir(true)}>Aprovar e publicar encontros</button>
