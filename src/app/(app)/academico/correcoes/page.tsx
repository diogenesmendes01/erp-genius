...
export default async function RevisoesCorrecoesPage({ searchParams }: { searchParams: Promise<{ pagina?: string }> }) {
  ...
  const p = Number((await searchParams).pagina ?? 1), r = await listarRevisoesPorCorrecao({ pagina: Number.isInteger(p) && p > 0 && p <= 100000 ? p : 1 });
  ...
  return <section className="space-y-5"><h1 className="text-2xl font-medium">Revisões após correções acadêmicas</h1>
    ...
    {d.itens.map(i => <article key={`${i.tipo}:${i.id}:${i.matricula.id}`} className="space-y-3 rounded border p-4"><h2 className="text-lg font-medium">{nomeCompleto(i.matricula.aluno)} · {i.matricula.codigo ?? "Matrícula sem código"}</h2>
      <p>{i.tipo === "AULA" ? i.labelAula ?? `Correção de aula — versão ${i.versaoCorrecao}` : i.tipo === "REPOSICAO" ? `${i.labelReposicao ?? "Conclusão de reposição"}: correção ${i.versaoCorrecao}` : i.tipo === "RECUPERACAO" ? `Recuperação: correção ${i.versaoCorrecao} de ${i.codigoAvaliacao}` : `Avaliação regular: correção ${i.versaoCorrecao} de ${i.codigoAvaliacao}`}, aplicada por {i.decisor} em {i.criadaEm.toLocaleString("pt-BR", { timeZone: "UTC" })} UTC.</p><p className="whitespace-pre-wrap">{i.motivo}</p>
