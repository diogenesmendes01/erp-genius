  ...
  return <section className="space-y-5" aria-label="Mudanças acadêmicas">
    ...
    {solicitacoes.map((p) => <article key={p.id} className="space-y-4 rounded-lg border border-gray-200 bg-surface p-4" aria-labelledby={`pedido-${p.id}`}>
      ...
      {p.podeDecidir && <div className="space-y-3 border-t border-gray-200 pt-3">
        <label className="block text-sm">Fundamentação da decisão<CampoTexto className={`${campo} mt-1`} rows={3} maxLength={2000} disabled={ocupado} value={valor(p.id, "decisao")} onChange={(e) => escrever(p.id, "decisao", e.target.value)} /></label>
        {!p.temParecerVigente && <label className="block text-sm">Justificativa para dispensar o parecer indisponível<span className="block text-xs text-gray-500">Obrigatória para aprovar sem parecer vigente do professor atual. A rejeição não exige dispensa.</span><CampoTexto className={`${campo} mt-1`} rows={2} maxLength={2000} disabled={ocupado} value={valor(p.id, "dispensa")} onChange={(e) => escrever(p.id, "dispensa", e.target.value)} /></label>}
      ...
      {p.podeExecutar && <div className="space-y-3 border-t border-gray-200 pt-3">
        <label className="block text-sm">Registro da execução<CampoTexto className={`${campo} mt-1`} rows={2} maxLength={2000} disabled={ocupado} value={valor(p.id, "execucao")} onChange={(e) => escrever(p.id, "execucao", e.target.value)} /></label>
      ...
      {p.podeCancelar && <details className="border-t border-gray-200 pt-3 text-sm"><summary className="cursor-pointer">Cancelar esta solicitação</summary><div className="mt-3 space-y-2"><p className="text-gray-500">O cancelamento preserva a turma atual e permite uma nova solicitação.</p><label className="block">Motivo do cancelamento<CampoTexto className={`${campo} mt-1`} rows={2} maxLength={2000} disabled={ocupado} value={valor(p.id, "cancelamento")} onChange={(e) => escrever(p.id, "cancelamento", e.target.value)} /></label><button className={botao} disabled={ocupado || valor(p.id, "cancelamento").trim().length < 5} onClick={() => void executar(() => cancelarMudancaAcademica(p.id, { motivo: valor(p.id, "cancelamento") }), "Solicitação cancelada. A turma atual foi preservada.")}>Confirmar cancelamento da solicitação</button></div></details>}
    </article>)}
  </section>;
}
