...
export default async function Designacao({ params, searchParams }: { params: Promise<{ itemReservaId: string }>; searchParams: Promise<{ buscaProfessor?: string; antesVersao?: string }> }) {
  ...
  const r = await consultarDesignacaoRecuperacao({ itemReservaId, buscaProfessor, ...(antesVersao ? { antesVersao: Number(antesVersao) } : {}) });
  ...
  return <section className="space-y-4">
    ...
    {(d.podeAlterar || d.podeConferirSubstituicao) && !antesVersao && <>
      ...
      {d.podeConferirSubstituicao && <PreviaSubstituicao key={d.buscaProfessor} itemReservaId={d.itemReservaId} atualId={d.avaliadorAgendaId} professores={d.professores} />}
    </>}
    <h2 className="text-xl font-medium">Histórico de designações</h2>
    {d.historico.map(h => <article key={h.id} className="rounded border p-3"><p>Versão {h.versao}: {h.professor?.nome ?? "Designação revogada"} — registrada por {h.gestor.nome}, em {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: "UTC" }).format(new Date(h.criadaEm))} (UTC).</p><p className="whitespace-pre-wrap">{h.motivo}</p></article>)}
