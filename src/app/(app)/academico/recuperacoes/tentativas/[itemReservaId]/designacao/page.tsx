...
export default async function Designacao({ params, searchParams }: { params: Promise<{ itemReservaId: string }>; searchParams: Promise<{ buscaProfessor?: string; antesVersao?: string }> }) {
  ...
  return <section className="space-y-4">
    ...
    {(d.podeAlterar || d.podeConferirSubstituicao) && !antesVersao && <>
      <form className="space-y-2"><label className="block">Buscar professor por nome<input name="buscaProfessor" defaultValue={d.buscaProfessor} maxLength={100} className="block rounded border p-2" /></label><button className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Buscar</button></form>
