...
export default async function AutorizacoesSegundaChamada({ params, searchParams }: { params: Promise<{ alocacaoId: string; codigoAvaliacao: string }>; searchParams: Promise<{ depoisId?: string }> }) {
  ...
  const resultado = await consultarAutorizacoesEspeciaisSegundaChamada({ alocacaoId, codigoAvaliacao, ...(depoisId ? { depoisId } : {}) });
  ...
  return <section className="space-y-4">
    ...
    {d.historico.map(autorizacao => <article key={autorizacao.id} className="space-y-1 rounded border p-3"><p>Autorizada por {autorizacao.autorizador.nome}, em {dataHoraUtc(autorizacao.criadaEm)} (UTC).</p><p>Prazo até {dataHoraUtc(autorizacao.prazoAte)} (UTC).</p><p className="whitespace-pre-wrap">{autorizacao.motivo}</p></article>)}
