...
export function Formulario({ reservaId, estadoConferido, proposta }: Props) {
 ...
 const [ocupado, setOcupado] = useState(false), [erro, setErro] = useState(""), [decisao, setDecisao] = useState(""), [inicioLocal, setInicioLocal] = useState("");
 ...
 <label className="block">Novo início<input name="inicioLocal" type="datetime-local" required onChange={e => setInicioLocal(e.target.value)} className="block border p-2" /></label>
 <label className="block">Novo término<input name="fimLocal" type="datetime-local" required min={inicioLocal || undefined} className="block border p-2" /></label>
