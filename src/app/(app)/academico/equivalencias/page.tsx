...
const dataHora = (valor: Date) => new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: "America/Sao_Paulo" }).format(valor);
...
export default async function EquivalenciasPage({ searchParams }: { searchParams: Promise<{ matriculaId?: string; cursor?: string }> }) {
  ...
  const resultado = await listarPropostasEquivalencia({ matriculaId, ...(cursor ? { cursor } : {}) });
  ...
  const lista = (propostas: typeof itens) => <div className="space-y-3">{propostas.map((proposta) => <article key={proposta.id} className="space-y-2 rounded border p-4">
    ...
    <p className="text-sm">Preparada em {dataHora(proposta.criadaEm)} (horário de São Paulo).</p>
