...
export function ExcecaoFrequencia({
  ...
  return <section className="space-y-3 rounded border p-4" aria-label="Exceção específica de frequência">
    ...
    {podePropor && <form className="space-y-3" onSubmit={propor}>
      ...
      <button type="submit" disabled={ocupado} className="rounded bg-brand-700 px-3 py-2 text-white disabled:opacity-50">{ocupado ? "Registrando…" : proposta ? "Registrar nova proposta" : "Registrar proposta"}</button>
    </form>}
    ...
    {podeDecidir && proposta && <form className="space-y-3" onSubmit={decidir}>
      ...
      <button type="submit" disabled={ocupado} className="rounded bg-brand-700 px-3 py-2 text-white disabled:opacity-50">{ocupado ? "Registrando…" : "Registrar decisão"}</button>
    </form>}
