...
export default async function EquivalenciaTransferenciaPage({ params }: { params: Promise<{ alocacaoId: string }> }) {
  ...
  if (!contexto.ok || !contexto.dado) return <section className="space-y-3"><VoltarPara href={`/academico/avaliacoes/${encodeURIComponent(alocacaoId)}`} para="Avaliações" /><p role="alert">{contexto.ok ? "Consulta indisponível." : contexto.erro}</p></section>;
  if (contexto.dado.origem?.alocacaoId !== alocacaoId) return <section className="space-y-3"><VoltarPara href={`/academico/avaliacoes/${encodeURIComponent(alocacaoId)}`} para="Avaliações" /><p role="alert">O vínculo de origem mudou. Atualize a matrícula antes de preparar o aproveitamento.</p></section>;
