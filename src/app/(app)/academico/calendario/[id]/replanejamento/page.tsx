...
export default async function ReplanejamentoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const resultado = await preverReplanejamentoCalendario({ calendarioId: id });
  ...
  return <div className="space-y-5">
    ...
    <p>Estas datas são sugestões. Nenhuma alteração foi aplicada à agenda. Cada turma mostra os horários no seu fuso de origem.</p>
    <EditorRevisao key={`${r.estadoHash}:${r.versaoRascunho}`} inicial={r} />
  </div>;
}
