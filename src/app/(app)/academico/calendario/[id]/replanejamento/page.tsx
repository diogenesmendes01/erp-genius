...
export default async function ReplanejamentoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!resultado.ok || !resultado.dado) return <div><VoltarPara href={`/academico/calendario/${id}`} para="Calendário" /><p role="alert">{resultado.ok ? "Revisão indisponível." : resultado.erro}</p></div>;
