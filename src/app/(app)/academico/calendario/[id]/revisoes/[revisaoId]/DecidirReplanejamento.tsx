...
export function DecidirReplanejamento({ calendarioId, revisaoId, podeAprovar, podeRejeitar, excecoes }: { calendarioId: string; revisaoId: string; podeAprovar: boolean; podeRejeitar: boolean; excecoes: Excecao[] }) {
 ...
 return <form className="space-y-3" action={(f) => { const acao = f.get("acao"); enviar(acao === "aprovar"); }}>
   ...
   {excecoes.map((e) => <label className="flex gap-2" key={e.encontroId}><input type="checkbox" disabled={pendente || !e.motivoProposto} checked={autorizar.includes(e.encontroId)} onChange={(x) => setAutorizar((v) => x.target.checked ? [...v, e.encontroId] : v.filter((id) => id !== e.encontroId))} />Autorizar exceção de {e.codigo ?? "turma identificada"} em {new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: e.fusoOrigem ?? "UTC" }).format(new Date(e.inicio))} ({e.fusoOrigem ?? "UTC"}){e.motivoProposto ? `: ${e.motivoProposto}` : " (sem justificativa)"}</label>)}
   {erro && <p role="alert">{erro}</p>}
   {(podeAprovar || podeRejeitar) ? <div className="flex gap-2">{podeAprovar && <button name="acao" value="aprovar" disabled={pendente} className="rounded bg-blue-700 px-3 py-2 text-white">{pendente ? "Registrando…" : "Aprovar e aplicar conjunto"}</button>}{podeRejeitar && <button name="acao" value="rejeitar" disabled={pendente} className="rounded border px-3 py-2">Rejeitar revisão</button>}</div> : <p>Esta pessoa não pode decidir o conjunto na situação atual.</p>}
 </form>;
}
