  ...
  return <div className="space-y-5">
    ...
    {r.ok && r.dado?.itens.map((a) => {
      ...
      return <article key={a.id} className="space-y-3 rounded-lg border bg-[var(--surface)] p-4">
        ...
        {a.encontrosPendentes.length > 0 && <div className="rounded bg-amber-50 dark:bg-amber-950 p-3"><h3 className="font-medium">Aulas que ainda precisam de solução</h3><ul className="list-inside list-disc">{a.encontrosPendentes.map((e) => <li key={e.id}>{exibicao(e.inicio).texto} — {exibicao(e.fim).texto}</li>)}</ul></div>}
