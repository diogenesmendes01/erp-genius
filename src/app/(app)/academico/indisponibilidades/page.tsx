...
export default async function IndisponibilidadesPage({ searchParams }: { searchParams: Promise<{ cursor?: string }> }) {
  ...
  return <div className="space-y-5">
    ...
    {r.ok && r.dado?.itens.map((a) => {
      const data = (v: string) => new Intl.DateTimeFormat("pt-BR", { dateStyle: "short", timeStyle: "short", timeZone: a.fusoOrigem }).format(new Date(v));
      return <article key={a.id} className="space-y-3 rounded-lg border bg-[var(--surface)] p-4">
        ...
        <p>{data(a.inicio)} — {data(a.fim)} <span className="text-sm text-gray-500">({a.fusoOrigem})</span></p>
        <p className="whitespace-pre-wrap text-sm">{a.motivo}</p>
        {a.decisao && <p className="text-sm">Decisão: {a.decisao.motivo}</p>}
        {a.situacao === "PENDENTE" && <div className="rounded bg-blue-50 dark:bg-blue-950 p-3"><h3 className="font-medium">Impacto para conferir antes da decisão</h3>
          {a.encontrosParaConferencia.length ? <><p className="text-sm">Estas aulas precisarão de solução se a ausência for aprovada.</p><ul className="list-inside list-disc">{a.encontrosParaConferencia.map((e) => <li key={e.id}>{data(e.inicio)} — {data(e.fim)}</li>)}</ul></> : <p className="text-sm">Nenhuma aula prevista coincide atualmente com o período solicitado.</p>}
          {a.reservasParaConferencia.length > 0 && <><p className="text-sm">Estes horários particulares reservados também precisarão de solução.</p><ul className="list-inside list-disc">{a.reservasParaConferencia.map((r) => <li key={r.id}>{data(r.inicio)} — {data(r.fim)}</li>)}</ul></>}
        </div>}
        {a.reservasPendentes.length > 0 && <div className="rounded bg-amber-50 dark:bg-amber-950 p-3"><h3 className="font-medium">Reservas particulares que ainda precisam de solução</h3><ul className="list-inside list-disc">{a.reservasPendentes.map((r) => <li key={r.id}>{data(r.inicio)} — {data(r.fim)}</li>)}</ul><p className="text-sm">A ausência não cancela a reserva nem escolhe outro professor.</p></div>}
        {a.encontrosPendentes.length > 0 && <div className="rounded bg-amber-50 dark:bg-amber-950 p-3"><h3 className="font-medium">Aulas que ainda precisam de solução</h3><ul className="list-inside list-disc">{a.encontrosPendentes.map((e) => <li key={e.id}>{data(e.inicio)} — {data(e.fim)}</li>)}</ul></div>}
