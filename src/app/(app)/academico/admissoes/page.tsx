...
export default async function AdmissoesPage({ searchParams }: { searchParams: Promise<{ busca?: string; pagina?: string }> }) {
  ...
  return <div className="space-y-4"><VoltarPara href="/academico" />
    ...
    <form><label>Buscar turma<input name="busca" defaultValue={busca} maxLength={100} className="mx-2 rounded border bg-[var(--surface)] p-2" /></label><button className={botaoClasses({ variante: "secundario", tamanho: "lg" })}>Buscar</button></form>
    {!turmas.length && <EstadoVazio bloco>Nenhuma turma encontrada.</EstadoVazio>}
