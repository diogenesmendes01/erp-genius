...
export function FusoExibicaoFormulario({ atual }: { atual: string | null }) {
  ...
  return <form className="space-y-3 rounded border bg-surface p-5" onSubmit={(e) => { e.preventDefault(); enviar(); }}>
    ...
    <button disabled={ocupado} className="rounded border px-4 py-2">{ocupado ? "Salvando…" : "Salvar preferência"}</button>{feito && <p role="status">{feito}</p>}{erro && <p role="alert" className="text-red-700">{erro}</p>}
  </form>;
}
