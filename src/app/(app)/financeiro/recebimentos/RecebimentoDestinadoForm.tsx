...
type Contexto = { matriculaId: string; status: string; aluno: string; moeda: string; cobrancas: { id: string; codigo: string | null; tipo: string; vencimento: string; saldo: number }[]; pagadores: { id: string; rotulo: string }[] };
...
export function RecebimentoDestinadoForm({ contextos }: { contextos: Contexto[] }) {
  ...
  return <div className="space-y-5 rounded-lg border p-5"><fieldset disabled={ocupado || Boolean(sucesso) || operacaoIncerta} className="space-y-5"><label className="block text-sm font-medium">Contrato e moeda<select aria-label="Contrato" className={input} value={matriculaId} onChange={(event) => selecionarContrato(event.target.value)}><option value="">Selecione o contrato</option>{contextos.map((item) => <option key={item.matriculaId} value={item.matriculaId}>{item.aluno} · {item.moeda}{item.status === "AGUARDANDO" ? " · em preparação" : ""}</option>)}</select></label>
    {contexto?.status === "AGUARDANDO" && <p role="status" className="rounded border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900">Matrícula em preparação: registre somente cobranças já emitidas ou uma antecipação respaldada pelo acordo e pela evidência. O recebimento não ativa o contrato.</p>}
    ...
    {!sucesso && <button type="button" disabled={ocupado || !contexto || total <= 0 || diferencaCentavos !== 0} className="rounded bg-brand-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" onClick={confirmar}>{ocupado ? "Registrando…" : operacaoIncerta ? "Tentar novamente" : "Confirmar recebimento"}</button>}{erro && <p role="alert" className="text-sm text-red-700">{erro}</p>}{sucesso && <><p role="status" className="text-sm text-green-700">{sucesso}</p><button type="button" className="rounded border px-3 py-2 text-sm" onClick={novoLancamento}>Novo lançamento</button></>}
  </div>;
}
