...
export function RecebimentoDestinadoForm({ contextos }: { contextos: Contexto[] }) {
  ...
  const chave = useRef(crypto.randomUUID()); const emEnvio = useRef(false); const [ocupado, setOcupado] = useState(false); const [erro, setErro] = useState<string | null>(null); const [sucesso, setSucesso] = useState<string | null>(null);
  ...
  return <div className="space-y-5 rounded-lg border p-5"><label className="block text-sm font-medium">Contrato e moeda<select aria-label="Contrato" className={input} value={matriculaId} onChange={(event) => selecionarContrato(event.target.value)}><option value="">Selecione o contrato</option>{contextos.map((item) => <option key={item.matriculaId} value={item.matriculaId}>{item.aluno} · {item.moeda}</option>)}</select></label>
    ...
    <label className="block text-sm font-medium">Documento / comprovante{exigeComprovante && <span className="text-red-700"> *</span>}<div className="mt-1">{contexto && <UploadArquivo label="Anexar comprovante" matriculaId={contexto.matriculaId} categoriaDocumento="COMPROVANTE" onUpload={(arquivo) => { setComprovanteUrl(arquivo.url); setComprovanteNome(arquivo.nome); }} />}{comprovanteUrl && <p className="text-xs text-green-700">{comprovanteNome || "Documento anexado"}</p>}</div></label><label className="block text-sm font-medium">Observação<input aria-label="Observação" className={input} value={comentario} onChange={(event) => setComentario(event.target.value)} /></label>
    <button type="button" disabled={ocupado || !contexto || total <= 0 || diferencaCentavos !== 0} className="rounded bg-brand-600 px-3 py-2 text-sm font-medium text-white disabled:opacity-50" onClick={confirmar}>{ocupado ? "Registrando…" : "Confirmar recebimento"}</button>{erro && <p role="alert" className="text-sm text-red-700">{erro}</p>}{sucesso && <p role="status" className="text-sm text-green-700">{sucesso}</p>}
  </div>;
}
