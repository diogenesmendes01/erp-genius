...
describe("entrada financeira histórica renderizada", () => {
  it("bloqueia preparação sem mapa de migração", () => { const html=renderToStaticMarkup(createElement(EntradaFinanceiraHistorica,{linhaId:"l",temMapa:false,moeda:"CRC",paises,propostas:[]})); expect(html).toContain("ainda não tem mapa de migração"); expect(html).not.toContain("Registrar proposta histórica"); });
  it("mostra a fotografia completa e decisão só para outro aprovador", () => { const html=renderToStaticMarkup(createElement(EntradaFinanceiraHistorica,{linhaId:"l",temMapa:true,moeda:"USD",paises,propostas:[proposta]})); expect(html).toContain('value="USD"'); expect(html).toContain("Vencimento"); expect(html).toContain("10/02/2025"); expect(html).toContain("competência 02/2025"); expect(html).toContain("Responsável"); expect(html).toContain("planilha"); expect(html).toContain("Aprovar e aplicar"); });
