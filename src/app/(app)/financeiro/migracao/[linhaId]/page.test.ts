...
describe("rota de conciliação financeira", () => { it("não exige abrir o lote administrativo", async () => { const html = renderToStaticMarkup(await Pagina({ params: Promise.resolve({ linhaId: "l" }), searchParams: Promise.resolve({}) })); expect(html).toContain("Voltar ao financeiro"); expect(html).toContain("formulário financeiro"); expect(html).toContain("entrada histórica da linha l"); }); });
