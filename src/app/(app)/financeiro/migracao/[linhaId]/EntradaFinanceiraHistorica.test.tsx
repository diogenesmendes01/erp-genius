...
describe("entrada financeira histórica renderizada", () => {
  ...
  it("mostra a fotografia completa e decisão só para outro aprovador", () => { const html=renderToStaticMarkup(createElement(EntradaFinanceiraHistorica,{linhaId:"l",temMapa:true,moeda:"USD",paises,propostas:[proposta]})); expect(html).toContain('value="USD"'); expect(html).toContain("Vencimento"); expect(html).toContain("competência 2025-02"); expect(html).toContain("Responsável"); expect(html).toContain("planilha"); expect(html).toContain("Aprovar e aplicar"); });
