...
export function ConferenciaFinanceiraMigracao({ dados, preferenciaFusoExibicao = null }: { dados: DadosConciliacaoFinanceira; preferenciaFusoExibicao?: string | null }) {
  ...
  return <section className="space-y-5"><header><h1 className="text-xl font-medium">Conciliação financeira migrada</h1><p className="text-sm text-gray-600">Linha {dados.linha.linhaOrigem} · origem {dados.linha.lote.origem} · contrato {dados.linha.mapa.codigo ?? "sem código"} de {dados.linha.mapa.aluno}.</p></header>
    ...
    {dados.resolucao ? <p role="status" className="rounded border border-emerald-300 bg-emerald-50 p-4">Origem conciliada com recebimento em {instanteAdministrativo(dados.resolucao.aplicadaEm)}. Eventual pendência anterior permanece no histórico e está resolvida.</p> : <section className="space-y-3 rounded border p-4"><h2 className="font-medium">{dados.pendenciaRegistrada ? "Resolver pendência com recebimento comprovado" : "Preparar proposta"}</h2><p className="text-sm text-gray-600">A seleção usa apenas cobranças, recebimentos e pagadores deste contrato. Uma pendência não cria recebimento.</p>
      ...
      {formulario.modalidade !== "PENDENCIA" && <fieldset className="space-y-3 rounded border border-amber-300 bg-amber-50 p-3"><legend className="px-1">Complemento estruturado por campo</legend>{camposComplementoFinanceiro.map((campo) => <div key={campo} className="grid gap-2 sm:grid-cols-3"><p className="text-xs"><strong>{campo}</strong><br/>Fonte: {fonteFinanceira(dados.linha.dadosOrigem, campo)}<br/>Proposto: {campo === "tipo" ? (cobranca?.tipo ?? "") : campo === "situacao" ? "PAGAMENTO_COMPROVADO" : campo === "dataPagamento" ? (dataHistoricaComOffset(formulario.dataLocal, formulario.offset) ?? "") : campo === "pagadorId" ? (dados.pagadores.find((pagador) => pagador.id === formulario.pagadorId) ? `${identificarPagador(dados.pagadores.find((pagador) => pagador.id === formulario.pagadorId)!)} · registro ${formulario.pagadorId}` : "") : campo === "valor" ? formulario.valor : campo === "moeda" ? formulario.moeda : formulario.forma}</p><label>Motivo<textarea aria-label={`Motivo do complemento: ${campo}`} value={formulario.detalhes[campo].motivo} onChange={(e) => mudarDetalhe(campo, "motivo", e.target.value)} /></label><label>Evidência<textarea aria-label={`Evidência do complemento: ${campo}`} value={formulario.detalhes[campo].evidencia} onChange={(e) => mudarDetalhe(campo, "evidencia", e.target.value)} /></label></div>)}</fieldset>}      <button type="button" disabled={ocupado} className="rounded bg-brand-solid px-3 py-2 text-white disabled:opacity-50" onClick={propor}>{ocupado ? "Registrando…" : "Registrar proposta"}</button>{erro && <p role="alert">{erro}</p>}{sucesso && <p role="status">{sucesso}</p>}
    </fieldset></section>}
