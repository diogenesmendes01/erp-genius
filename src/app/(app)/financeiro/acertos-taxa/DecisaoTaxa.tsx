...
import { decidirAcertoTaxaAditivo, aplicarAcertoTaxaAditivo } from "@/server/contratos/aditivo-acerto-taxa-acoes";
export function DecisaoTaxa({ propostaId, podeDecidir, podeAplicar }: { propostaId: string; podeDecidir: boolean; podeAplicar: boolean }) {
  ...
  async function enviar(operacao: "aprovar" | "rejeitar" | "aplicar") {
    if (enviando.current || (operacao === "aplicar" ? !podeAplicar : !podeDecidir || motivo.trim().length < 5)) return;
