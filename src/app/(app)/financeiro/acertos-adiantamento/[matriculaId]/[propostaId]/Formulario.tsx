...
import { proporAdiantamentoAditivo, decidirAdiantamentoAditivo, aplicarAdiantamentoAditivo } from "@/server/contratos/adiantamento-aditivo";
...
export function AdiantamentoFormulario(props: Props) {
 ...
 return <form className="space-y-3 rounded border p-3" onSubmit={async evento => {
  ...
  try {
   ...
   const r = await (props.modo === "preparar" ? proporAdiantamentoAditivo(entrada) : props.modo === "decidir" ? decidirAdiantamentoAditivo(entrada) : aplicarAdiantamentoAditivo(entrada));
 ...
 <button disabled={ocupado || concluido} className="rounded border px-3 py-2" type="submit">{concluido ? "Registrado" : ocupado ? "Processando…" : preservada ? "Repetir mesma tentativa" : props.modo === "preparar" ? "Preparar acerto" : props.modo === "decidir" ? "Registrar decisão" : "Aplicar adiantamento aprovado"}</button>
