...
type Conjunto = { id: string; status: string; podeDecidir: boolean; podeAplicar: boolean; podeObsoletar: boolean; pendencias: { afetadasSemAplicacao: number }; impactos: { cobrancaId: string; classificacao: string; justificativa: string; aplicado: boolean; coberturaInicioNova: string | null; coberturaFimNova: string | null; cobranca: { codigo: string | null; moeda: string; coberturaInicio: string | null; coberturaFim: string | null; vencimento: string; status: string } }[] };
...
export function ImpactosCoberturaOperacao({ conjunto, reprepararHref }: { conjunto: Conjunto | null; reprepararHref: string }) {
  ...
  return <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Conjunto de cobertura · {conjunto.status}</h2>{conjunto.impactos.map(i => <article key={i.cobrancaId} className="rounded border p-3"><p className="font-medium">{i.cobranca.codigo ?? "Mensalidade sem código"} · {i.classificacao}</p><p>Atual: {i.cobranca.coberturaInicio ?? "não informada"} até {i.cobranca.coberturaFim ?? "não informada"}. {i.classificacao === "AFETADA" && <>Corrigida: {i.coberturaInicioNova} até {i.coberturaFimNova}.</>}</p><p>{i.justificativa}</p><p role="status">{i.aplicado ? "Aplicação registrada." : i.classificacao === "AFETADA" ? "Aguardando aplicação." : "Preservada sem aplicação."}</p></article>)}
