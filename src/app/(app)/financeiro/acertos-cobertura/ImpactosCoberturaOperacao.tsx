...
export function ImpactosCoberturaOperacao({ conjunto, reprepararHref }: { conjunto: Conjunto | null; reprepararHref: string }) {
  ...
  return <section className="space-y-3 rounded border p-4"><h2 className="text-xl">Conjunto de cobertura · {conjunto.status}</h2><p>Política formalizada no aditivo assinado: {descreverPolitica(conjunto.politica)}</p><p>Motivo do preparo: {conjunto.motivo}</p><p>Evidência da conferência: {conjunto.evidencia}</p>{conjunto.decisao && <p>Decisão registrada: {conjunto.decisao.aprovada ? "aprovada" : "rejeitada"}. Motivo: {conjunto.decisao.motivo}</p>}{conjunto.impactos.map(i => <article key={i.cobrancaId} className="rounded border p-3"><p className="font-medium">{i.cobranca.codigo ?? "Mensalidade sem código"} · {i.classificacao}</p><p>Cobertura anterior: {formatarDataCivil(i.coberturaInicioAnterior, "não informada")} até {formatarDataCivil(i.coberturaFimAnterior, "não informada")}. {i.classificacao === "AFETADA" && <>Resultado proposto: {formatarDataCivil(i.coberturaInicioNova)} até {formatarDataCivil(i.coberturaFimNova)}.</>}</p><p>{i.justificativa}</p><p role="status">{i.aplicado ? "Aplicação registrada." : i.classificacao === "AFETADA" ? "Aguardando aplicação." : "Preservada sem aplicação."}</p></article>)}
    ...
    {conjunto.podeDecidir && <><button className={botaoClasses()} type="button" disabled={ocupado || motivo.trim().length < 5} onClick={() => executar("aprovar")}>Aprovar conjunto</button>{" "}<button className={botaoClasses({ variante: "secundario" })} type="button" disabled={ocupado || motivo.trim().length < 5} onClick={() => executar("rejeitar")}>Rejeitar conjunto</button></>}
    ...
    {conjunto.podeObsoletar && <button className={botaoClasses({ variante: "secundario" })} type="button" disabled={ocupado || motivo.trim().length < 5} onClick={() => executar("obsoletar")}>{conjunto.status === "APROVADO" ? "Confirmar divergência material" : "Descartar conjunto pendente"}</button>}
