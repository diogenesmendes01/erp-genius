...
export default async function AcertosCoberturaPage({ searchParams }: { searchParams: Promise<{ pagina?: string }> }) {
  ...
  return <main className="space-y-4"><VoltarPara href="/financeiro" /><h1 className="text-2xl">Correções de cobertura por aditivo</h1><p>Prepare todas as mensalidades, registre a decisão independente e aplique o conjunto completo uma única vez.</p>
    {!r.ok ? <p role="alert">{r.erro}</p> : <><ul className="space-y-3">{r.dado?.itens.map(item => <li key={item.propostaId}><Link className="underline" href={`/financeiro/acertos-cobertura/${encodeURIComponent(item.matriculaId)}/${encodeURIComponent(item.propostaId)}`}>Matrícula {item.matriculaId} · condições versão {item.versao}</Link></li>)}</ul>{!r.dado?.itens.length && <EstadoVazio role="status">Nenhum aditivo de cobertura foi encontrado nesta página.</EstadoVazio>}<nav className="flex gap-4">{pagina > 1 && <Link href={`?pagina=${pagina - 1}`}>Anterior</Link>}{r.dado?.temProxima && <Link href={`?pagina=${pagina + 1}`}>Próxima</Link>}</nav></>}
  </main>;
}
