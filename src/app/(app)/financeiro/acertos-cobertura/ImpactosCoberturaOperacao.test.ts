...
function montar(status: string, motivo = "") {
  ...
  return ImpactosCoberturaOperacao({ conjunto: { id: "conjunto", status, podeDecidir: status === "PENDENTE", podeAplicar: status === "APROVADO", podeObsoletar: status === "PENDENTE", pendencias: { afetadasSemAplicacao: 1 }, impactos: [{ cobrancaId: "m1", classificacao: "AFETADA", justificativa: "Limites corrigidos pelo aditivo.", aplicado: false, coberturaInicioNova: "2026-11-01", coberturaFimNova: "2026-11-30", cobranca: { codigo: "M-1", moeda: "CRC", coberturaInicio: "2026-10-01", coberturaFim: "2026-10-31", vencimento: "2026-10-01", status: "PENDENTE" } }] }, reprepararHref: "/repreparar" });
}
it("não oferece mutações quando a consulta não autoriza operação", () => {
  ...
  const elemento = ImpactosCoberturaOperacao({ conjunto: { id: "conjunto", status: "PENDENTE", podeDecidir: false, podeAplicar: false, podeObsoletar: false, pendencias: { afetadasSemAplicacao: 1 }, impactos: [] }, reprepararHref: "/repreparar" });
