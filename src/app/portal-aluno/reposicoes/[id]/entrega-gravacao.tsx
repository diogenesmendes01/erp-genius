...
export function EntregaGravacaoPortalAluno({ reposicaoId, podeEntregar, motivoBloqueio }: { reposicaoId: string; podeEntregar: boolean; motivoBloqueio: string | null }) {
  ...
  if (!podeEntregar) return <p className="mt-5 rounded border bg-white p-4 text-sm text-gray-700">{motivoBloqueio ?? "Não há uma entrega aberta neste momento."}</p>;
  return <form onSubmit={enviar} className="mt-5 space-y-3 rounded border bg-white p-5"><h2 className="font-medium">Enviar resumo e atividade</h2>
