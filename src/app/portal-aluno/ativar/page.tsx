...
export default function AtivarPortalAlunoPage() {
  ...
  return <section className="mx-auto flex min-h-screen max-w-md items-center px-5"><div className="w-full rounded-xl border bg-white p-7 shadow-sm"><h1 className="text-2xl font-medium">Definir senha</h1><p className="mt-2 text-sm text-gray-600">Crie uma senha que só você conhece.</p><form onSubmit={confirmar} className="mt-5 space-y-4"><label className="block text-sm">Nova senha<input required minLength={12} maxLength={72} name="senha" type="password" autoComplete="new-password" className="mt-1 w-full rounded border px-3 py-2" /></label>{erro && <p role="alert" className="text-sm text-red-700">{erro}</p>}<button disabled={enviando} className="w-full rounded bg-brand-600 px-4 py-2 text-sm font-medium text-white disabled:opacity-60">{enviando ? "Confirmando…" : "Confirmar acesso"}</button></form></div></section>;
}
