...
CREATE TABLE "PropostaEntradaFinanceiraHistoricaMigracao" (
  ...
  vencimento date NOT NULL, competencia text CHECK (competencia IS NULL OR competencia ~ '^\\d{4}-\\d{2}$'),
...
BEGIN
 ...
 IF TG_OP='INSERT' AND NEW.status<>'PENDENTE' THEN RAISE EXCEPTION 'Proposta M01 deve iniciar pendente'; END IF;
 ...
 IF NEW.status IN ('APROVADA','REJEITADA') AND (NEW."decisorId" IS NULL OR NEW."decisorId"=NEW."preparadorId" OR NEW."chaveDecisao" IS NULL OR btrim(NEW."motivoDecisao")='' OR NEW."decididoEm" IS NULL) THEN RAISE EXCEPTION 'M01 exige decisão independente, motivo, chave e data'; END IF;
 ...
 IF jsonb_typeof(NEW.evidencia)<>'object' OR NEW.evidencia='{}'::jsonb OR jsonb_typeof(NEW."dadosPagador")<>'object' OR NEW."dadosPagador"->>'tipo' NOT IN ('ALUNO','RESPONSAVEL','EMPRESA') OR jsonb_typeof(NEW."dadosPagador"->'dados')<>'object' OR btrim(NEW."dadosPagador"->'dados'->>'nome')='' OR btrim(NEW."dadosPagador"->'dados'->>'paisId')='' THEN RAISE EXCEPTION 'Pagador e evidência estruturados são obrigatórios'; END IF;
 RETURN NEW;
END $$;
...
BEGIN
 ...
 IF p.status IS DISTINCT FROM 'APROVADA' OR p."decisorId" IS DISTINCT FROM NEW."aplicadaPorId" OR p.origem IS DISTINCT FROM NEW.origem OR p."financeiroOrigemId" IS DISTINCT FROM NEW."financeiroOrigemId" OR c."matriculaId" IS DISTINCT FROM p."matriculaId" OR pg."matriculaId" IS DISTINCT FROM p."matriculaId" OR c.status<>'PENDENTE' OR c."valorRecebido" IS NOT NULL OR c.saldo IS DISTINCT FROM c."valorNegociado" OR c."valorNegociado" IS DISTINCT FROM p.valor OR c.moeda IS DISTINCT FROM p.moeda OR c.tipo IS DISTINCT FROM p."tipoCobranca" THEN RAISE EXCEPTION 'Aplicação M01 deve materializar apenas pagador e obrigação pendente aprovada'; END IF;
