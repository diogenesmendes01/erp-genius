...
BEGIN
 ...
 IF TG_OP='INSERT' AND (NEW.status<>'PENDENTE' OR NEW."decisorId" IS NOT NULL OR NEW."chaveDecisao" IS NOT NULL OR NEW."motivoDecisao" IS NOT NULL OR NEW."decididoEm" IS NOT NULL OR NEW."aplicadaEm" IS NOT NULL) THEN RAISE EXCEPTION 'Proposta M01 deve iniciar pendente sem decisão ou aplicação'; END IF;
 ...
 IF TG_OP='UPDATE' AND OLD.status='APROVADA' AND (OLD."decisorId",OLD."chaveDecisao",OLD."motivoDecisao",OLD."decididoEm") IS DISTINCT FROM (NEW."decisorId",NEW."chaveDecisao",NEW."motivoDecisao",NEW."decididoEm") THEN RAISE EXCEPTION 'Decisão M01 aprovada é imutável'; END IF;
 IF NEW.status IN ('APROVADA','REJEITADA','APLICADA') AND (NEW."decisorId" IS NULL OR NEW."decisorId"=NEW."preparadorId" OR NULLIF(btrim(NEW."chaveDecisao"),'') IS NULL OR NULLIF(btrim(NEW."motivoDecisao"),'') IS NULL OR NEW."decididoEm" IS NULL) THEN RAISE EXCEPTION 'M01 exige decisão independente, motivo, chave e data'; END IF;
