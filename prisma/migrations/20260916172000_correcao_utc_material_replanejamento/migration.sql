...
BEGIN
 ...
 IF v_total=0 OR v_total IS DISTINCT FROM v_evento_total OR EXISTS (
   ...
   ) OR NOT EXISTS (SELECT 1 FROM "EncontroAgenda" a WHERE a.id=(h->>'encontroId') AND a.inicio=(((h->>'inicioProposto')::timestamptz) AT TIME ZONE 'UTC') AND a.fim=(((h->>'fimProposto')::timestamptz) AT TIME ZONE 'UTC'))
