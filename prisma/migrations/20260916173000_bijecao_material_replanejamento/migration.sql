...
BEGIN
 ...
 IF v_total=0 OR v_total IS DISTINCT FROM v_evento_total OR jsonb_array_length(COALESCE(v_evento->'encontrosIds','[]'::jsonb)) IS DISTINCT FROM v_total OR EXISTS (SELECT 1 FROM jsonb_array_elements_text(v_evento->'encontrosIds') i GROUP BY i HAVING count(*)>1) OR EXISTS (SELECT 1 FROM jsonb_array_elements(v_evento->'horarios') h GROUP BY h->>'encontroId' HAVING count(*)>1) OR EXISTS (
   SELECT 1 FROM jsonb_array_elements(v_snapshot->'revisoes') t CROSS JOIN LATERAL jsonb_array_elements(COALESCE(t->'previsao'->'propostas','[]'::jsonb)) p WHERE COALESCE((p->>'alterado')::boolean,false) AND (NOT (v_evento->'encontrosIds' ? (p->>'encontroId')) OR NOT EXISTS (SELECT 1 FROM jsonb_array_elements(v_evento->'horarios') h WHERE h->>'encontroId'=p->>'encontroId' AND h->>'inicioProposto'=p->>'inicioProposto' AND h->>'fimProposto'=p->>'fimProposto') OR NOT EXISTS (SELECT 1 FROM "EncontroAgenda" a WHERE a.id=p->>'encontroId' AND a.inicio=((p->>'inicioProposto')::timestamptz AT TIME ZONE 'UTC') AND a.fim=((p->>'fimProposto')::timestamptz AT TIME ZONE 'UTC')))
