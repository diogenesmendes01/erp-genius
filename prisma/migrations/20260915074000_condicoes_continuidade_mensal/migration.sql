-- Condições versionadas da continuidade mensal. A emissão por período é um
-- fato posterior: esta tabela só registra a transcrição e a decisão interna.
CREATE TABLE "CondicoesContinuidadeMensalMatricula" (
    "id" TEXT NOT NULL,
    ...
    CONSTRAINT "CondicoesContinuidadeMensalMatricula_pkey" PRIMARY KEY ("id")
);

CREATE UNIQUE INDEX "CondContinuidadeMensal_matricula_versao_key"
  ON "CondicoesContinuidadeMensalMatricula"("matriculaId", "versao");
CREATE INDEX "CondContinuidadeMensal_matricula_status_idx"
  ON "CondicoesContinuidadeMensalMatricula"("matriculaId", "status");

ALTER TABLE "CondicoesContinuidadeMensalMatricula"
  ADD CONSTRAINT "CondContinuidadeMensal_matricula_fkey"
  FOREIGN KEY ("matriculaId") REFERENCES "Matricula"("id") ON DELETE RESTRICT ON UPDATE NO ACTION;
ALTER TABLE "CondicoesContinuidadeMensalMatricula"
  ADD CONSTRAINT "CondContinuidadeMensal_documento_fkey"
  FOREIGN KEY ("documentoId") REFERENCES "Documento"("id") ON DELETE RESTRICT ON UPDATE NO ACTION;
ALTER TABLE "CondicoesContinuidadeMensalMatricula"
  ADD CONSTRAINT "CondContinuidadeMensal_preparador_fkey"
  FOREIGN KEY ("preparadorId") REFERENCES "Usuario"("id") ON DELETE RESTRICT ON UPDATE NO ACTION;
ALTER TABLE "CondicoesContinuidadeMensalMatricula"
  ADD CONSTRAINT "CondContinuidadeMensal_decisor_fkey"
  FOREIGN KEY ("decisorId") REFERENCES "Usuario"("id") ON DELETE RESTRICT ON UPDATE NO ACTION;

CREATE FUNCTION validar_condicoes_continuidade_mensal() RETURNS trigger LANGUAGE plpgsql AS $$
...
BEGIN
  ...
  IF TG_OP = 'INSERT' OR NEW.status = 'APROVADA' THEN
    ...
    IF EXISTS (SELECT 1 FROM "PreparacaoComercialMatricula" WHERE "matriculaId" = m.id AND regime <> 'MENSALIDADE') THEN
      RAISE EXCEPTION 'Condições exigem contratação mensal.';
    END IF;
...
CREATE TRIGGER validar_condicoes_continuidade_mensal
BEFORE INSERT OR UPDATE OR DELETE ON "CondicoesContinuidadeMensalMatricula"
FOR EACH ROW EXECUTE FUNCTION validar_condicoes_continuidade_mensal();
