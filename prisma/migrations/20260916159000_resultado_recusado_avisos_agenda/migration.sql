...
BEGIN
  IF TG_OP = 'INSERT' THEN
    ...
    IF NOT EXISTS (SELECT 1 FROM "Evento" e WHERE e.id=NEW."eventoId" AND e.payload->>'aprovada'='true' AND ((e."agregadoTipo"='Matricula' AND e."agregadoId"=NEW."matriculaId" AND e.tipo IN ('RemarcacaoParticularDecidida','RemarcacaoAgendaReposicaoDecidida') AND e.payload ? 'encontroOriginalId' AND e.payload ? 'encontroNovoId' AND e.payload->>'encontroNovoId' IS NOT NULL) OR (e."agregadoTipo"='ConfiguracaoOperacional' AND e.tipo='SubstituicaoDocenteDecidida' AND e.payload ? 'encontrosIds'))) THEN RAISE EXCEPTION 'Origem do aviso inválida'; END IF;
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM "AvisoAlteracaoAgenda" a JOIN "EncontroAgenda" encontro ON encontro.id=NEW."encontroId" JOIN "Evento" evento ON evento.id=a."eventoId" WHERE a.id=NEW."avisoId" AND a.situacao='PREPARADO' AND encontro."matriculaId"=a."matriculaId" AND (NEW."encontroId" IN (evento.payload->>'encontroOriginalId', evento.payload->>'encontroNovoId') OR evento.payload->'encontrosIds' ? NEW."encontroId")) THEN RAISE EXCEPTION 'Item não pertence à alteração aplicada'; END IF;
  RETURN NEW;
END $$;
