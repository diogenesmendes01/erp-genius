...
BEGIN
 ...
 IF NOT EXISTS (SELECT 1 FROM "DecisaoCalendarioEscolar" c WHERE c."calendarioId"=v_calendario AND c.aprovada AND c."decisorId"=v_decisor) THEN
 ...
 SELECT count(*) INTO v_total FROM jsonb_array_elements(v_snapshot->'revisoes') t, jsonb_array_elements(COALESCE(t->'previsao'->'propostas','[]'::jsonb)) p WHERE COALESCE((p->>'alterado')::boolean,false);
 ...
 IF v_total=0 OR v_total IS DISTINCT FROM v_evento_total OR EXISTS (
   SELECT 1 FROM jsonb_array_elements(v_evento->'horarios') h WHERE NOT EXISTS (
     SELECT 1 FROM jsonb_array_elements(v_snapshot->'revisoes') t, jsonb_array_elements(COALESCE(t->'previsao'->'propostas','[]'::jsonb)) p
     WHERE COALESCE((p->>'alterado')::boolean,false) AND p->>'encontroId'=h->>'encontroId' AND p->>'inicioAnterior'=h->>'inicioAnterior' AND p->>'fimAnterior'=h->>'fimAnterior' AND p->>'inicioProposto'=h->>'inicioProposto' AND p->>'fimProposto'=h->>'fimProposto'
