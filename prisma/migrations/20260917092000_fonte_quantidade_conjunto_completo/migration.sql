-- Q38/197: completa 196 sem a editar. O payload da fonte deve ser a lista exata
-- de encontros materializados a partir dos snapshots publicados.
CREATE OR REPLACE FUNCTION quantidade_aviso_conjunto_completo(evento_id text)
RETURNS boolean LANGUAGE plpgsql STABLE AS $$
DECLARE evento record; proposta_id text; invalido boolean;
BEGIN
 SELECT e.*,p.id AS proposta_id INTO evento FROM "Evento" e
 ...
 IF NOT FOUND THEN RETURN false; END IF; proposta_id:=evento.proposta_id;
 SELECT EXISTS (SELECT 1 FROM "ImpactoQuantidadeAulasModalidade" i WHERE i."propostaId"=proposta_id AND i.publicada AND (
 ...
 RETURN NOT EXISTS (
  ...
  adicionados AS (SELECT f.turma_id,n.id encontro_id FROM fotos_adicionadas f JOIN "EncontroAgenda" n ON n."matriculaId" IS NULL AND n."turmaId"=f.turma_id AND n."chaveIdempotencia"=('quantidade:'||proposta_id||':'||f.turma_id||':'||f.indice::text) WHERE n.inicio=((f.item->>'inicio')::timestamptz AT TIME ZONE 'UTC') AND n.fim=((f.item->>'fim')::timestamptz AT TIME ZONE 'UTC') AND n.status::text=f.item->>'status' AND n."professorId" IS NOT DISTINCT FROM f.item->>'professorId' AND n."propostaGradeId" IS NOT DISTINCT FROM f.item->>'propostaGradeId'),
...
-- Recria somente o guard de cabeçalho de 196: todos os guards de item permanecem.
