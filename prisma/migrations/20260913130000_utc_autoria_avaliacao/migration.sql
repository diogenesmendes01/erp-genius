CREATE OR REPLACE FUNCTION conferir_autoria_lancamento(registro_id TEXT, registrador_id TEXT, realizador_id TEXT, realizada_em TIMESTAMP, motivo TEXT, evidencias TEXT) RETURNS void LANGUAGE plpgsql AS $$
...
BEGIN
 ...
 IF designado_id IS DISTINCT FROM registrador_id AND NOT EXISTS (
  ...
  WHERE t.id = r."turmaId" AND t."professorId" = registrador_id AND t.status <> 'CONCLUIDA' AND vd.fim IS NULL AND vd.inicio <= (clock_timestamp() AT TIME ZONE 'UTC')
...
BEGIN
 ...
 IF TG_TABLE_NAME = 'VersaoLancamentoAvaliacao' THEN
  ...
  IF NEW."realizadaEm" > (clock_timestamp() AT TIME ZONE 'UTC') OR NEW."realizadaEm" < a."criadoEm" OR (a."encerradaEm" IS NOT NULL AND NEW."realizadaEm" >= a."encerradaEm") OR (NOT a.ativa AND a."encerradaEm" IS NULL) THEN RAISE EXCEPTION 'Data fora do vínculo histórico da avaliação'; END IF;
