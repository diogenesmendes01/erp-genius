-- Q151: designação de segunda chamada só substitui a atribuição ordinária na
-- nota original da própria realização, enquanto a versão vigente a confirma.
...
BEGIN
 ...
 SELECT d."professorId" INTO professor_atual FROM "DesignacaoSegundaChamada" d
   WHERE d."propostaId"=proposta_id
     AND d."criadaEm"<=(clock_timestamp() AT TIME ZONE 'UTC')
     AND d.inicio<=(clock_timestamp() AT TIME ZONE 'UTC')
     AND (d.fim IS NULL OR d.fim>(clock_timestamp() AT TIME ZONE 'UTC'))
   ORDER BY d.versao DESC LIMIT 1 FOR SHARE;
 SELECT d."professorId" INTO professor_historico FROM "DesignacaoSegundaChamada" d
   WHERE d."propostaId"=proposta_id
     AND d."criadaEm"<=realizada_em AND d.inicio<=realizada_em
     AND (d.fim IS NULL OR d.fim>realizada_em)
   ORDER BY d.versao DESC LIMIT 1 FOR SHARE;
...
BEGIN
 ...
 IF TG_TABLE_NAME = 'VersaoLancamentoAvaliacao' THEN
  ...
  autoria_segunda:=NEW."segundaChamadaRealizacaoId" IS NOT NULL AND autoria_segunda_chamada_valida(r.id,NEW."segundaChamadaRealizacaoId",NEW."autorId",NEW."realizadaPorId",NEW."realizadaEm");
  IF autoria_segunda IS DISTINCT FROM true THEN
