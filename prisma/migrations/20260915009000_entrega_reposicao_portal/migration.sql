...
BEGIN
  ...
  IF aluno_matricula IS NULL OR NEW."alunoId" <> aluno_matricula OR conta_aluno IS DISTINCT FROM aluno_matricula
...
CREATE TRIGGER "EntregaReposicaoGravacao_validar_origem_portal" BEFORE INSERT ON "EntregaReposicaoGravacao"
  FOR EACH ROW WHEN (NEW."contaPortalAlunoId" IS NOT NULL) EXECUTE FUNCTION "validar_origem_entrega_reposicao_portal"();
