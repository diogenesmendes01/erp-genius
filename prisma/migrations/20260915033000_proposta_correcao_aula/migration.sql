-- Q23: a proposta é um fato imutável. Nesta etapa não existe decisão nem
-- publicação: AulaDiario, RegistroAulaAluno e EncontroAgenda permanecem
-- inalterados até a futura aprovação independente.

CREATE TABLE "PropostaCorrecaoAula" (
  "id" TEXT NOT NULL,
  ...
  CONSTRAINT "PropostaCorrecaoAula_pkey" PRIMARY KEY ("id"),
  CONSTRAINT "PropostaCorrecaoAula_versao_positiva_check" CHECK ("versao" > 0),
  CONSTRAINT "PropostaCorrecaoAula_snapshotAnterior_objeto_check" CHECK (jsonb_typeof("snapshotAnterior") = 'object'),
  CONSTRAINT "PropostaCorrecaoAula_snapshotNovo_objeto_check" CHECK (jsonb_typeof("snapshotNovo") = 'object'),
  CONSTRAINT "PropostaCorrecaoAula_estadoHash_formato_check" CHECK ("estadoHash" ~ '^[a-f0-9]{64}$'),
  CONSTRAINT "PropostaCorrecaoAula_entradaHash_formato_check" CHECK ("entradaHash" ~ '^[a-f0-9]{64}$'),
  CONSTRAINT "PropostaCorrecaoAula_motivo_tamanho_check" CHECK (length(btrim("motivo")) BETWEEN 5 AND 3000),
  CONSTRAINT "PropostaCorrecaoAula_evidencia_tamanho_check" CHECK (length(btrim("evidencia")) BETWEEN 5 AND 3000),
  CONSTRAINT "PropostaCorrecaoAula_chave_tamanho_check" CHECK (length("chaveIdempotencia") BETWEEN 8 AND 100)
);

CREATE UNIQUE INDEX "PropostaCorrecaoAula_encontro_versao_key"
  ON "PropostaCorrecaoAula" ("encontroId", "versao");
CREATE UNIQUE INDEX "PropostaCorrecaoAula_autor_chave_key"
  ON "PropostaCorrecaoAula" ("autorId", "chaveIdempotencia");
CREATE INDEX "PropostaCorrecaoAula_diario_criada_idx"
  ON "PropostaCorrecaoAula" ("diarioId", "criadaEm");

ALTER TABLE "PropostaCorrecaoAula"
  ADD CONSTRAINT "PropostaCorrecaoAula_encontroId_fkey"
    FOREIGN KEY ("encontroId") REFERENCES "EncontroAgenda"("id") ON DELETE RESTRICT ON UPDATE NO ACTION,
  ADD CONSTRAINT "PropostaCorrecaoAula_diarioId_fkey"
    FOREIGN KEY ("diarioId") REFERENCES "AulaDiario"("id") ON DELETE RESTRICT ON UPDATE NO ACTION,
  ADD CONSTRAINT "PropostaCorrecaoAula_autorId_fkey"
...
BEGIN
  ...
  SELECT decisao.id INTO decisao_excecao_id
    ...
    LIMIT 1;
  IF (decisao_excecao_id IS NULL AND jsonb_typeof(NEW."snapshotAnterior" -> 'gravacao') IS DISTINCT FROM 'null')
...
CREATE TRIGGER "PropostaCorrecaoAula_validar"
  BEFORE INSERT OR UPDATE OR DELETE ON "PropostaCorrecaoAula"
  FOR EACH ROW EXECUTE FUNCTION "validar_proposta_correcao_aula"();
