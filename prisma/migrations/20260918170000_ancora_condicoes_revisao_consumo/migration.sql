-- 261/262 are immutable. Anchor the enrollment before reading conditions so
-- validar_condicoes_horas serializes a new applicable condition with Q23 photo.
-- This preserves the established calendar advisory -> encounter -> enrollment order.
...
BEGIN
  ...
  PERFORM matricula_ancora.id FROM "Matricula" matricula_ancora
    WHERE matricula_ancora.id=encontro_registro."matriculaId" FOR UPDATE;
